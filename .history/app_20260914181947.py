...
class PartyApp:
    ...
    def __init__(self, root):
        ...
        tk.Label(
            input_frame, text="خولی هەڵبژاردن:", font=("Arial", 10), bg="#f0f0f0"
        ).grid(row=0, column=0, padx=5, pady=5, sticky="w")
        
        self.cycle_entry = ttk.Combobox(
            input_frame,
            values=["خولی پەرلەمانی کوردستان", "خولی پەرلەمانی عێراق"],
            width=20,
            state="readonly"
        )
        self.cycle_entry.grid(row=0, column=1, padx=5, pady=5)
        self.cycle_entry.set("خولی پەرلەمانی کوردستان")
        ...
        # بەشی ناوەڕاست: کۆمبۆ بۆکسی خول و چارتەکان (پای و دۆنەت) لە ناو بەشی ئەنجامدا
        ...
        tk.Label(
            chart_control_frame,
            text="دیاریکردنی خول بۆ چارت:",
            font=("Arial", 10, "bold"),
            bg="#f0f0f0",
        ).pack(side="left", padx=5)

        self.filter_cycle_var = tk.StringVar(value="خولی پەرلەمانی کوردستان")
        ...
        self.tree.heading("Cycle", text="خولی هەڵبژاردن")
    ...
    def update_chart(self):
        ...
        conn = sqlite3.connect("parties_data.db")
        cursor = conn.cursor()
        cursor.execute("SELECT party_name, votes, color FROM parties WHERE cycle = ?", (selected_cycle,))
