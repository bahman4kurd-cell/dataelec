...
with st.sidebar.form("party_form"):
  cycle = st.selectbox(
      "حکومی / جۆر:", [ئەنجومەنی پارێزگاکان", "پەرلەمانی کوردستان", "پەرلەمانی عێراق"]
  )
...
with col1:
  ...
  if rows:
    ...
    if st.button("סڕینەوەی هەڵبژاردە", type="primary"):
      conn = sqlite3.connect("parties_data.db")
      cursor = conn.cursor()
...
with col2:
  ...
  filter_cycle = cycle = st.selectbox(
      "حکومی / جۆر:", ["حکومی", "ئەنجومەنی پارێزگاکان", "پەرلەمانی کوردستان", "پەرلەمانی عێراق"]
  )
