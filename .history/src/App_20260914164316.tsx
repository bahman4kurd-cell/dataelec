...
type ChartType = 'bars' | 'progress' | 'summary';
...
export function App() {
  ...
  // پاشەکەوتکردنی جۆری چارت بۆ هەر خولێک بە پێی IDـی خولەکە
  ...
  const fetchRounds = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setRounds(data);
        // دیاریکردنی چاری سەرەتایی بۆ هەموو خولەکان
        const initialCharts: { [key: number]: ChartType } = {};
        data.forEach((r: ElectionRound) => {
          initialCharts[r.id] = 'progress';
        });
      ...
      setRoundChartTypes({ 1: 'progress', 2: 'bars' });
    }
  };
  ...
  // گۆڕینی جۆری چارت بۆ خولێکی دیاریکراو
  const handleChartTypeChange = (roundId: number, type: ChartType) => {
    setRoundChartTypes(prev => ({
      ...prev,
      [roundId]: type
    }));
  };
                  ...
                  return (
                    <div key={round.id} className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-6 shadow-sm space-y-5">
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-[var(--border-color)]">
                        ...
                        {/* کۆمبۆبۆکسی هەڵبژاردنی جۆری چارت */}
                        <div className="flex items-center gap-2">
                          ...
                          >
                            <option value="progress">📊 هێڵکاری ڕێژەیی (Progress Bars)</option>
                            <option value="bars">📈 هێڵکاری ستوونی (Vertical Bars)</option>
                            <option value="summary">📋 کارتی کورتەی ئەنجامەکان (Summary)</option>
                          </select>
                        </div>
                      </div>

                      {/* ناوەڕۆکی چارتەکە بە پێی هەڵبژاردنی کۆمبۆبۆکسەکە */}
                      <div>
                        ...
                        {currentChart === 'summary' && (
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            {partyResults.map((party, index) => (
                              <div key={party.id} className="bg-[var(--bg-main)] border border-[var(--border-color)] p-4 rounded-lg flex items-center gap-3">
                                <div className={`w-10 h-10 rounded-lg ${party.color} flex items-center justify-center text-white font-bold`}>
                                  #{index + 1}
                                </div>
                                <div>
                                  <h4 className="font-bold text-sm">{party.name}</h4>
                                  <p className="text-xs text-[var(--text-secondary)] mt-0.5">{party.votes.toLocaleString()} دەنگ</p>
                                </div>
                              </div>
                            ))}
                          </div>
