...
type ChartType = 'pie' | 'donut' | 'bars' | 'progress';
...
export function App() {
  ...
  // کۆمبۆ بۆکسی جۆری چارت لە داشبۆرد
  ...
  useEffect(() => {
    ...
    // نموونەی سەرەتایی
    const initialRounds: ElectionRound[] = [
      { id: 1, name: 'هەڵبژاردنی پەرلەمانی کوردستان 2024', date: '2024-10-20', type: 'پەرلەمانی', totalVoters: 2899578, status: 'چالاک' },
      { id: 2, name: 'هەڵبژاردنی ئەنجومەنی پارێزگاکان', date: '2023-12-18', type: 'ئەنجومەنی پارێزگاکان', totalVoters: 2500000, status: 'تەواوبوو' }
    ];
    setRounds(initialRounds);
    setSelectedRoundId(1);
    setDashSelectedRoundId(1);

    const initialBranches: Branch[] = [
      { id: 1, roundId: 1, name: 'لقی هەولێر' },
      { id: 2, roundId: 1, name: 'لقی سلێمانی' },
      { id: 3, roundId: 2, name: 'لقی کەرکووک' }
    ];
    ...
    const initialRegions: Region[] = [
      ...
      { id: 3, branchId: 2, name: 'بنکەی سەیدسادق' }
    ];
    setRegions(initialRegions);
    setSelectedRegionId(1);
    
    setRegionVotes({
      1: defaultParties.map(p => ({ ...p, votes: 45000, percentage: 50 })),
      2: defaultParties.map(p => ({ ...p, votes: 30000, percentage: 33.3 })),
      3: defaultParties.map(p => ({ ...p, votes: 15000, percentage: 16.7 }))
    });
  }, [theme]);
  ...
  // گۆڕینی دەنگ و حسابکردنی ڕێژەی سەدی
  ...
  // کۆکردنەوەی دەنگەکان بەپێی فلتەری داشبۆرد
  const getFilteredDashboardVotes = () => {
    ...
    // کۆکردنەوەی گشتی دەنگەکان بۆ پارتەکان لە ناوچە دیاریکراوەکاندا
        ...
        <main className="flex-1 p-6 overflow-y-auto space-y-6">
          {activeMainTab === 'dashboard' && (
            <div className="space-y-6">
              <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-6 shadow-sm space-y-6">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-[var(--border-color)] pb-4">
                  <div>
                    ...
                    <p className="text-sm text-[var(--text-secondary)] mt-1">ئەنجامی دەنگەکان بەپێی فلتەری خول، لق و ناوچەکان</p>
                  </div>

                  {/* کۆمبۆ بۆکسی جۆری چارت */}
                  <div className="flex items-center gap-2">
                    <label className="text-sm font-medium text-[var(--text-secondary)]">جۆری هێڵکاری:</label>
                    <select
                      value={dashboardChartType}
                      onChange={(e) => setDashboardChartType(e.target.value as ChartType)}
                      className="bg-[var(--bg-main)] border border-[var(--border-color)] rounded-lg px-3 py-1.5 text-sm font-semibold"
                    >
                      <option value="bars">📊 هێڵکاری ستوونی</option>
                      <option value="pie">🥧 هێڵکاری بازنەیی (Pie)</option>
                      <option value="donut">🍩 هێڵکاری دۆنات (Donut)</option>
                      <option value="progress">📈 پیشاندەری ڕێژەیی (Progress)</option>
                    </select>
                  </div>
                </div>

                {/* تابەکانی خولەکان بۆ داشبۆرد */}
                <div className="space-y-3">
                  <span className="text-xs font-bold text-[var(--text-secondary)]">دیاریکردنی خولی هەڵبژاردن بۆ هێڵکارییەکان:</span>
              ...
              {/* نمای هێڵکارییەکان بەپێی هەڵبژاردن */}
              <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-6 shadow-sm space-y-6">
                <div className="flex justify-between items-center">
                  <h3 className="text-lg font-bold">ئەنجامی دەنگەکان (کۆی گشتی دەنگ: {totalDashboardVotes.toLocaleString()})</h3>
                </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2">
                    {dashboardData.map(item => (
                      <div key={item.partyName} className="bg-[var(--bg-main)] border border-[var(--border-color)] p-5 rounded-xl flex flex-col items-center justify-center text-center gap-3 shadow-sm">
                        ...
                        <div className="text-2xl font-extrabold text-blue-600">{item.percentage}%</div>
                ...
                {(dashboardChartType === 'pie' || dashboardChartType === 'donut') && (
                  <div className="flex flex-col md:flex-row items-center justify-center gap-8 py-6">
                    <div className={`relative w-48 h-48 rounded-full flex items-center justify-center shadow-lg border-4 border-[var(--border-color)] ${dashboardChartType === 'donut' ? 'bg-[var(--bg-main)]' : 'bg-blue-600'}`}>
                      {dashboardChartType === 'donut' && (
                        <div className="absolute w-28 h-28 bg-[var(--bg-card)] rounded-full flex flex-col items-center justify-center text-center">
                      ...
                      {dashboardChartType === 'pie' && (
                        <div className="text-white text-center font-bold">
                          ...
                          <div className="text-xs opacity-80">{totalDashboardVotes.toLocaleString()} دەنگ</div>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="space-y-3 w-full md:w-auto">
                      {dashboardData.map(item => (
                        <div key={item.partyName} className="flex items-center gap-3 bg-[var(--bg-main)] border border-[var(--border-color)] px-4 py-2 rounded-lg">
                          ...
                          <span className="text-xs text-[var(--text-secondary)]">({item.votes.toLocaleString()})</span>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-6 shadow-sm space-y-6">
                  ...
                  {selectedBranchId && (
                    <div className="mt-6 pt-6 border-t border-[var(--border-color)] space-y-4">
                      ...
                      {/* فۆڕمی داتا ئینتری دەنگەکان بۆ هەر پارتێک */}
                      {selectedRegionId && (
