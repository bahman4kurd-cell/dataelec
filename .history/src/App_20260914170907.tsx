...
export function App() {
  ...
  // داتاکانی خولەکان (بەبێ داتای نموونەیی پێشوەختە)
  ...
  // کۆمبۆ بۆکسی جۆری چارت و فلتەرەکان لە داشبۆرد
  ...
  // هەڵبژاردنی یەکەم خول بە شێوەی ئۆتۆماتیکی ئەگەر هەبوو
  useEffect(() => {
  ...
  // کۆکردنەوەی دەنگەکان بەپێی فلتەری داشبۆرد و خولی هەڵبژێردراو
            ...
            <div className="space-y-6">
              <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-6 shadow-sm space-y-6">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-[var(--border-color)] pb-4">
                  ...
                  {/* کۆمبۆ بۆکسی جۆری چارتەکان بە تەواوی */}
                  <div className="flex items-center gap-2">
                ...
                {/* تابەکانی خولەکان لە داشبۆرددا بۆ گۆڕینی خێرای چارتەکان */}
              ...
              <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-6 shadow-sm space-y-6">
                ...
                {(dashboardChartType === 'pie' || dashboardChartType === 'donut') && (
                  <div className="flex flex-col md:flex-row items-center justify-center gap-8 py-6">
                    ...
                    <div className="space-y-3 w-full md:w-auto">
                      {dashboardData.map(item => (
                        <div key={item.partyName} className="flex items-center gap-3 bg-[var(--bg-main)] border border-[var(--border-color)] px-4 py-2.5 rounded-lg shadow-sm">
                ...
                {/* ٤. هێڵکاری گەشەسەندن (Line Chart Style) */}
                {dashboardChartType === 'line' && (
                  <div className="bg-[var(--bg-main)] border border-[var(--border-color)] p-6 rounded-xl space-y-6">
                    ...
                    <div className="flex items-end justify-around h-48 pt-6 border-b border-x border-[var(--border-color)] px-4 relative">
                      {dashboardData.map((item) => (
                        <div key={item.partyName} className="flex flex-col items-center gap-2 h-full justify-end group">
                          <span className="text-xs font-bold text-blue-600">{item.percentage}%</span>
                          <div
                            className={`w-12 rounded-t-lg ${item.color} transition-all duration-500 shadow-md group-hover:opacity-90`}
                            style={{ height: `${Math.max(item.percentage, 10)}%` }}
                          ></div>
                          <span className="text-xs font-semibold text-center mt-2 max-w-[100px] truncate">{item.partyName}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {/* ٥. هێڵکاری تۆڕی پێشکەوتوو (Network Nodes Graph) */}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      {dashboardData.map((item, idx) => (
                        <div key={item.partyName} className="relative bg-[var(--bg-card)] border-2 border-dashed border-[var(--border-color)] p-5 rounded-2xl flex flex-col items-center text-center gap-3 shadow-sm">
                          <div className={`absolute -top-3 right-4 px-3 py-0.5 text-xs font-bold text-white rounded-full ${item.color}`}>
                            گرێی #{idx + 1}
                          </div>
                          <span className={`w-6 h-6 rounded-full ${item.color} animate-pulse mt-2`}></span>
                ...
                {/* ٦. هێڵکاری پەیڤەری و تیشکی (Radial Gauge / Radar) */}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      {dashboardData.map(item => (
                        <div key={item.partyName} className="bg-[var(--bg-card)] border border-[var(--border-color)] p-6 rounded-2xl flex flex-col items-center justify-center text-center gap-4 shadow-sm">
                          <span className={`w-3.5 h-3.5 rounded-full ${item.color}`}></span>
                          <h5 className="font-bold text-sm">{item.partyName}</h5>
                          <div className="relative w-32 h-32 rounded-full border-8 border-[var(--border-color)] flex items-center justify-center">
                            <div className="absolute inset-0 rounded-full border-8 border-blue-600 opacity-20"></div>
