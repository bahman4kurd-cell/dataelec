...
export function App() {
  ...
  const [newRegionValidVotes, setNewRegionValidVotes] = useState('');
  const [newRegionSpoiledVotes, setNewRegionSpoiledVotes] = useState('');
  const [newRegionTotalVoters, setNewRegionTotalVoters] = useState('');
  ...
  // لایەنەکان بە ناوی فەرمی و ڕەنگە دیاریکراوەکانیان
  ...
  const handleSaveRegion = (e: React.FormEvent) => {
    ...
    if (editingRegionId !== null) {
      setRegions(regions.map(reg => reg.id === editingRegionId ? { 
        ...reg, 
        name: newRegionName,
        totalValidVotes: Number(newRegionValidVotes) || reg.totalValidVotes,
        spoiledVotes: Number(newRegionSpoiledVotes) || reg.spoiledVotes,
        totalVoters: Number(newRegionTotalVoters) || reg.totalVoters
      } : reg));
      ...
      const newReg: Region = { 
        ...
        name: newRegionName,
        totalValidVotes: Number(newRegionValidVotes) || 0,
        spoiledVotes: Number(newRegionSpoiledVotes) || 0,
        totalVoters: Number(newRegionTotalVoters) || 0
      };
    ...
    setNewRegionName('');
    setNewRegionValidVotes('');
    setNewRegionSpoiledVotes('');
    setNewRegionTotalVoters('');
  };

  const handleEditRegion = (reg: Region) => {
    setEditingRegionId(reg.id);
    setNewRegionName(reg.name);
    setNewRegionValidVotes(reg.totalValidVotes ? reg.totalValidVotes.toString() : '');
    setNewRegionSpoiledVotes(reg.spoiledVotes ? reg.spoiledVotes.toString() : '');
    setNewRegionTotalVoters(reg.totalVoters ? reg.totalVoters.toString() : '');
  };
  ...
  const selectedRegionObj = regions.find(reg => reg.id === selectedRegionId);
    ...
    <div className="min-h-screen flex flex-col bg-[var(--bg-main)] text-[var(--text-primary)] transition-colors duration-200" dir="rtl">
      <header className="h-16 bg-[var(--bg-card)] border-b border-[var(--border-color)] px-6 flex items-center justify-between shadow-md">
        ...
        <div className="flex items-center gap-2">
          <label className="text-sm text-[var(--text-secondary)] font-medium"> ڕووکار:</label>
          <select
            value={theme}
                    ...
                    <div className="mt-6 pt-6 border-t border-[var(--border-color)] space-y-4">
                      <div className="flex flex-col gap-4">
                        <div>
                          ...
                          <h3 className="text-lg font-bold mt-2">زیادکردن و بەڕێوەبردنی ناوچەکان و دەنگە گشتییەکان</h3>
                        </div>

                        {/* فرمی زیادکردن و دەستکاری ناوچە لەگەڵ ٣ تێکست بۆکسە داواکراوەکە */}
                        <form onSubmit={handleSaveRegion} className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-3 bg-[var(--bg-main)] p-4 rounded-xl border border-[var(--border-color)]">
                          <div>
                            <label className="block text-xs font-medium mb-1 text-[var(--text-secondary)]">ناوی ناوچە/بنکە</label>
                            <input
                              type="text"
                              value={newRegionName}
                              onChange={(e) => setNewRegionName(e.target.value)}
                              placeholder="ناوی ناوچە..."
                              className="w-full bg-[var(--bg-card)] border border-[var(--border-color)] rounded-lg px-3 py-2 text-sm"
                              required
                            />
                          </div>
                          <div>
                            <label className="block text-xs font-medium mb-1 text-[var(--text-secondary)]">کۆی گشتی دەنگی دروست</label>
                            <input
                              type="number"
                              value={newRegionValidVotes}
                              onChange={(e) => setNewRegionValidVotes(e.target.value)}
                              placeholder="0"
                              className="w-full bg-[var(--bg-card)] border border-[var(--border-color)] rounded-lg px-3 py-2 text-sm"
                            />
                          </div>
                          <div>
                            <label className="block text-xs font-medium mb-1 text-[var(--text-secondary)]">کۆی گشتی دەنگی سوتاو</label>
                            <input
                              type="number"
                              value={newRegionSpoiledVotes}
                              onChange={(e) => setNewRegionSpoiledVotes(e.target.value)}
                              placeholder="0"
                              className="w-full bg-[var(--bg-card)] border border-[var(--border-color)] rounded-lg px-3 py-2 text-sm"
                            />
                          </div>
                          <div>
                            <label className="block text-xs font-medium mb-1 text-[var(--text-secondary)]">کۆی گشتی دەنگدەر</label>
                            <input
                              type="number"
                              value={newRegionTotalVoters}
                              onChange={(e) => setNewRegionTotalVoters(e.target.value)}
                              placeholder="0"
                              className="w-full bg-[var(--bg-card)] border border-[var(--border-color)] rounded-lg px-3 py-2 text-sm"
                            />
                          </div>
                          <div className="flex items-end gap-2">
                            {editingRegionId !== null && (
                              <button
                                type="button"
                                onClick={() => { setEditingRegionId(null); setNewRegionName(''); setNewRegionValidVotes(''); setNewRegionSpoiledVotes(''); setNewRegionTotalVoters(''); }}
                                className="bg-gray-500 text-white px-3 py-2 rounded-lg text-xs font-semibold"
                              >
                                پاشگەزبوونەوە
                              </button>
                            )}
                            <button type="submit" className="w-full bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg text-sm font-semibold transition-all">
                        ...
                        {currentRegions.length === 0 ? (
                          ...
                          currentRegions.map(reg => (
                            <div
                              ...
                              className={`px-3.5 py-2 rounded-lg text-xs font-semibold cursor-pointer border flex items-center gap-3 transition-all ${
                            ...
                            >
                              <div className="flex flex-col">
                                ...
                                <span className={`text-[10px] ${selectedRegionId === reg.id ? 'text-green-100' : 'text-[var(--text-secondary)]'}`}>
                                  دەنگی دروست: {reg.totalValidVotes || 0} | سوتاو: {reg.spoiledVotes || 0} | دەنگدەر: {reg.totalVoters || 0}
                                </span>
                              </div>
                              <div className="flex gap-2 items-center" onClick={(e) => e.stopPropagation()}>
                                <button onClick={() => handleEditRegion(reg)} className="opacity-80 hover:opacity-100">✏️</button>
                        ...
                        <div className="mt-6 bg-[var(--bg-main)] border border-[var(--border-color)] rounded-xl p-5 space-y-4">
                          <div className="flex justify-between items-center border-b border-[var(--border-color)] pb-3">
                            <h4 className="font-bold text-base text-[var(--text-primary)]">
                              داتا ئینتری دەنگەکانی لایەنەکان بۆ ناوچەی: <span className="text-blue-600">{selectedRegionObj?.name}</span>
                            </h4>
                            <div className="flex gap-4 text-xs font-bold text-[var(--text-secondary)]">
                              <span>دەنگی دروست: {selectedRegionObj?.totalValidVotes || 0}</span>
                              <span>دەنگی سوتاو: {selectedRegionObj?.spoiledVotes || 0}</span>
                              <span>کۆی دەنگدەر: {selectedRegionObj?.totalVoters || 0}</span>
                            </div>
                          </div>
