          ...
          {activeMainTab === 'dashboard' && (
            <div className="space-y-6">
              ...
              <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-6 shadow-sm space-y-6">
                ...
                {/* هێڵکاری بازنەیی و دۆنەت بە SVG ڕاستەقینە */}
                    ...
                    <div className="relative w-64 h-64 flex items-center justify-center">
                      <svg viewBox="0 0 100 100" className="w-full h-full transform -rotate-90">
                        {(() => {
                          ...
                          return dashboardData.map((party) => {
                            const percent = totalDashboardVotes > 0 ? party.votes / totalDashboardVotes : 0;
                            const dashArray = `${percent * 283} 283`;
                            const dashOffset = -cumulativePercent * 283;
                            cumulativePercent += percent;

                            if (party.votes === 0) return null;
                            ...
                            return (
                              <circle
                                ...
                                r="45"
                                fill="transparent"
                                stroke={party.hexColor}
                                strokeWidth={dashboardChartType === 'donut' ? "22" : "45"}
                      ...
                      {dashboardChartType === 'donut' && (
                        <div className="absolute inset-0 flex flex-col items-center justify-center text-center pointer-events-none p-2">
                          <span className="text-xs font-medium text-[var(--text-secondary)]">کۆی گشتی</span>
                          <span className="text-base font-bold text-[var(--text-primary)]">{totalDashboardVotes.toLocaleString()}</span>
                        </div>
                      )}
                    </div>
