...
export function App() {
  ...
  const defaultParties = [
    ...
    { partyId: 8, partyName: 'بزووتنەوەی ئیسلامی', color: 'bg-slate-100 text-black', hexColor: '#64748b' },
    { partyId: 9, partyName: 'سۆسیالیست', color: 'bg-sky-300', hexColor: '#38bdf8' },
    { partyId: 10, partyName: 'هاوپەیمانی نیشتمانی', color: 'bg-emerald-300', hexColor: '#34d399' },
  ...
  // فانکشنی دروستکردنی کەرتی بازنەیی (SVG Arc) بۆ پای چارت و دۆنەت چارت بە شێوەیەکی زانستی و بێگەرد
  const createSvgSlices = (data: { partyName: string; votes: number; percentage: number; hexColor: string }[], isDonut: boolean) => {
    ...
    return data.map((item, index) => {
      ...
      if (Math.abs(angle - 360) < 0.1) {
        // ئەگەر تەنها یەک لایەن هەبوو و سەد لە سەد بوو
        return (
      ...
      // حسابکردنی شوێنی نیشاندانی ڕێژەی سەدی لە ناوەڕاستی کەرتی بازنەکەدا
          ...
          {activeMainTab === 'dashboard' && (
            <div className="space-y-6">
              ...
              <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-6 shadow-sm space-y-6">
                ...
                {/* پای چارت و دۆنەت چارت بە شێوازی SVG Path زانستی و پاک */}
                {(dashboardChartType === 'pie' || dashboardChartType === 'donut') && (
                  <div className="flex flex-col md:flex-row items-center justify-center gap-10 py-6">
                    ...
                    <div className="space-y-2.5 w-full md:w-80 max-h-80 overflow-y-auto px-2">
                      {dashboardData.map(item => (
                        <div key={item.partyName} className="flex items-center justify-between bg-[var(--bg-main)] border border-[var(--border-color)] px-3.5 py-2 rounded-lg shadow-sm text-xs">
