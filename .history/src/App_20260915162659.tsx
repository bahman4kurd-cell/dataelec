...
export function App() {
  const [users, setUsers] = useState<UserAccount[]>(() => {
    const saved = localStorage.getItem('election_users');
  ...
  const [currentUser, setCurrentUser] = useState<UserAccount | null>(() => {
    const saved = localStorage.getItem('election_current_user');
  ...
  // New user form states (for Super Admin Control Panel)
  ...
  useEffect(() => {
    if (currentUser) {
      localStorage.setItem('election_current_user', JSON.stringify(currentUser));
    } else {
      localStorage.removeItem('election_current_user');
    }
  }, [currentUser]);
  ...
  const handleDeleteUser = (id: number) => {
    if (id === 1) return; // Cannot delete main admin
  ...
  // Handle Branch Admin automatic restrictions
  useEffect(() => {
