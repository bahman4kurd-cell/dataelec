...
export function App() {
  ...
  const [users, setUsers] = useState<UserAccount[]>(() => {
    try {
      const saved = localStorage.getItem('election_users_v2');
  ...
  const [currentUser, setCurrentUser] = useState<UserAccount | null>(() => {
    try {
      const saved = localStorage.getItem('election_current_user_v2');
      if (saved) return JSON.parse(saved);
    } catch (e) {
      console.error(e);
    }
  ...
  useEffect(() => {
    localStorage.setItem('election_users_v2', JSON.stringify(users));
  }, [users]);

  useEffect(() => {
    if (currentUser) {
      localStorage.setItem('election_current_user_v2', JSON.stringify(currentUser));
    } else {
      localStorage.removeItem('election_current_user_v2');
    }
  }, [currentUser]);
  ...
  const handleCreateAccount = (e: React.FormEvent) => {
    ...
    if (!newAccUsername || !newAccPassword || !newAccName) return;

    if (users.some(u => u.username === newAccUsername)) {
      setLoginError('ئەم ناوی بەکارهێنەرە پێشتر هەیە!');
      return;
    }
    ...
    const newUser: UserAccount = {
      ...
      name: newAccName,
      role: newAccRole,
      branchId: newAccRole === 'branch_admin' ? newAccBranchId : null
    };

    setUsers([...users, newUser]);
    setAccSuccessMsg('ئەکاونتەکە بە سەرکەوتوویی دروستکرا!');
    setNewAccUsername('');
    setNewAccPassword('');
    setNewAccName('');
  ...
  const [rounds, setRounds] = useState<ElectionRound[]>(() => {
    try {
      const saved = localStorage.getItem('election_rounds_v2');
      if (saved) return JSON.parse(saved);
    } catch (e) {
      console.error(e);
    }
  ...
  const [selectedRoundId, setSelectedRoundId] = useState<number | null>(() => {
    try {
      const saved = localStorage.getItem('election_selected_round_v2');
      if (saved) return JSON.parse(saved);
    } catch (e) {
      console.error(e);
    }
  ...
  const [branches, setBranches] = useState<Branch[]>(() => {
    try {
      const saved = localStorage.getItem('election_branches_v2');
      if (saved) return JSON.parse(saved);
    } catch (e) {
      console.error(e);
    }
  ...
  const [selectedBranchId, setSelectedBranchId] = useState<number | null>(() => {
    try {
      const saved = localStorage.getItem('election_selected_branch_v2');
      if (saved) return JSON.parse(saved);
    } catch (e) {
      console.error(e);
    }
  ...
  const [regions, setRegions] = useState<Region[]>(() => {
    try {
      const saved = localStorage.getItem('election_regions_v2');
      if (saved) return JSON.parse(saved);
    } catch (e) {
      console.error(e);
    }
  ...
  const [selectedRegionId, setSelectedRegionId] = useState<number | null>(() => {
    try {
      const saved = localStorage.getItem('election_selected_region_v2');
      if (saved) return JSON.parse(saved);
    } catch (e) {
      console.error(e);
    }
  ...
  const [regionVotes, setRegionVotes] = useState<{ [regionId: number]: PartyVote[] }>(() => {
    try {
      const saved = localStorage.getItem('election_region_votes_v2');
      if (saved) return JSON.parse(saved);
    } catch (e) {
      console.error(e);
    }
  ...
  const [branchVotes, setBranchVotes] = useState<{ [branchId: number]: PartyVote[] }>(() => {
    try {
      const saved = localStorage.getItem('election_branch_votes_v2');
      if (saved) return JSON.parse(saved);
    } catch (e) {
      console.error(e);
    }
  ...
  const [dashSelectedRoundId, setDashSelectedRoundId] = useState<number | null>(() => {
    try {
      const saved = localStorage.getItem('election_dash_round_v2');
      if (saved) return JSON.parse(saved);
    } catch (e) {
      console.error(e);
    }
  ...
  useEffect(() => {
    localStorage.setItem('election_rounds_v2', JSON.stringify(rounds));
  }, [rounds]);

  useEffect(() => {
    localStorage.setItem('election_branches_v2', JSON.stringify(branches));
  }, [branches]);

  useEffect(() => {
    localStorage.setItem('election_regions_v2', JSON.stringify(regions));
  }, [regions]);

  useEffect(() => {
    localStorage.setItem('election_region_votes_v2', JSON.stringify(regionVotes));
  }, [regionVotes]);

  useEffect(() => {
    localStorage.setItem('election_branch_votes_v2', JSON.stringify(branchVotes));
  }, [branchVotes]);

  useEffect(() => {
    if (selectedRoundId !== null) {
      localStorage.setItem('election_selected_round_v2', JSON.stringify(selectedRoundId));
    }
  }, [selectedRoundId]);

  useEffect(() => {
    if (dashSelectedRoundId !== null) {
      localStorage.setItem('election_dash_round_v2', JSON.stringify(dashSelectedRoundId));
    }
  }, [dashSelectedRoundId]);
