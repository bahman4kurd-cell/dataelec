...
export function App() {
  // Safe LocalStorage Initialization to prevent data loss on reloads/updates
  const [users, setUsers] = useState<UserAccount[]>(() => {
    try {
      const saved = localStorage.getItem('election_users_v3');
  ...
  const [currentUser, setCurrentUser] = useState<UserAccount | null>(() => {
    try {
      const saved = localStorage.getItem('election_current_user_v3');
      if (saved) return JSON.parse(saved);
    } catch (e) {
      console.error(e);
    }
  ...
  useEffect(() => {
    localStorage.setItem('election_users_v3', JSON.stringify(users));
  }, [users]);

  useEffect(() => {
    if (currentUser) {
      localStorage.setItem('election_current_user_v3', JSON.stringify(currentUser));
    } else {
      localStorage.removeItem('election_current_user_v3');
    }
  }, [currentUser]);
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    // پشکنینی ورد بۆ یوزەر و پاسۆرد بە بێ جیاکاری گەورە و بچووکی پیتەکان لە یوزەرنەیمدا
    ...
    if (foundUser) {
      ...
      setLoginError('ناوی بەکارهێنەر یان وشەی نهێنی هەڵەیە! تکایە دڵنیابەوە لە زانیارییەکان.');
    }
  };
  ...
  const handleCreateAccount = (e: React.FormEvent) => {
    ...
    if (users.some(u => u.username.toLowerCase() === newAccUsername.toLowerCase())) {
      setLoginError('ئەم ناوی بەکارهێنەرە (Username) پێشتر هەیە، تکایە یوزەرنەیمێکی تر بەکاربهێنە!');
      return;
    }
    ...
    const newUser: UserAccount = {
      ...
      role: newAccRole,
      branchId: newAccRole === 'branch_admin' ? Number(newAccBranchId) : null
    };

    const updatedUsers = [...users, newUser];
    setUsers(updatedUsers);
    localStorage.setItem('election_users_v3', JSON.stringify(updatedUsers));

    setAccSuccessMsg('ئەکاونتەکە بە سەرکەوتوویی دروستکرا و הוچووە ناو سیستەمەوە!');
    setNewAccUsername('');
    setNewAccPassword('');
    setNewAccName('');
  ...
  const handleDeleteUser = (id: number) => {
    if (id === 1) return; // ئەکاونتی سەرەکی ئەدمن ناڕووخێنرێت
    const updated = users.filter(u => u.id !== id);
    setUsers(updated);
    localStorage.setItem('election_users_v3', JSON.stringify(updated));
  };
  ...
  const [rounds, setRounds] = useState<ElectionRound[]>(() => {
    try {
      const saved = localStorage.getItem('election_rounds_v3');
      if (saved) return JSON.parse(saved);
    } catch (e) {
      console.error(e);
    }
  ...
  const [selectedRoundId, setSelectedRoundId] = useState<number | null>(() => {
    try {
      const saved = localStorage.getItem('election_selected_round_v3');
      if (saved) return JSON.parse(saved);
    } catch (e) {
      console.error(e);
    }
  ...
  const [branches, setBranches] = useState<Branch[]>(() => {
    try {
      const saved = localStorage.getItem('election_branches_v3');
      if (saved) return JSON.parse(saved);
    } catch (e) {
      console.error(e);
    }
  ...
  const [selectedBranchId, setSelectedBranchId] = useState<number | null>(() => {
    try {
      const saved = localStorage.getItem('election_selected_branch_v3');
      if (saved) return JSON.parse(saved);
    } catch (e) {
      console.error(e);
    }
  ...
  const [regions, setRegions] = useState<Region[]>(() => {
    try {
      const saved = localStorage.getItem('election_regions_v3');
      if (saved) return JSON.parse(saved);
    } catch (e) {
      console.error(e);
    }
  ...
  const [selectedRegionId, setSelectedRegionId] = useState<number | null>(() => {
    try {
      const saved = localStorage.getItem('election_selected_region_v3');
      if (saved) return JSON.parse(saved);
    } catch (e) {
      console.error(e);
    }
  ...
  const [regionVotes, setRegionVotes] = useState<{ [regionId: number]: PartyVote[] }>(() => {
    try {
      const saved = localStorage.getItem('election_region_votes_v3');
      if (saved) return JSON.parse(saved);
    } catch (e) {
      console.error(e);
    }
  ...
  const [branchVotes, setBranchVotes] = useState<{ [branchId: number]: PartyVote[] }>(() => {
    try {
      const saved = localStorage.getItem('election_branch_votes_v3');
      if (saved) return JSON.parse(saved);
    } catch (e) {
      console.error(e);
    }
  ...
  const [dashSelectedRoundId, setDashSelectedRoundId] = useState<number | null>(() => {
    try {
      const saved = localStorage.getItem('election_dash_round_v3');
      if (saved) return JSON.parse(saved);
    } catch (e) {
      console.error(e);
    }
  ...
  useEffect(() => {
    localStorage.setItem('election_rounds_v3', JSON.stringify(rounds));
  }, [rounds]);

  useEffect(() => {
    localStorage.setItem('election_branches_v3', JSON.stringify(branches));
  }, [branches]);

  useEffect(() => {
    localStorage.setItem('election_regions_v3', JSON.stringify(regions));
  }, [regions]);

  useEffect(() => {
    localStorage.setItem('election_region_votes_v3', JSON.stringify(regionVotes));
  }, [regionVotes]);

  useEffect(() => {
    localStorage.setItem('election_branch_votes_v3', JSON.stringify(branchVotes));
  }, [branchVotes]);

  useEffect(() => {
    if (selectedRoundId !== null) {
      localStorage.setItem('election_selected_round_v3', JSON.stringify(selectedRoundId));
    }
  }, [selectedRoundId]);

  useEffect(() => {
    if (dashSelectedRoundId !== null) {
      localStorage.setItem('election_dash_round_v3', JSON.stringify(dashSelectedRoundId));
    }
  }, [dashSelectedRoundId]);
  ...
  const currentBranches = currentUser?.role === 'branch_admin' && currentUser.branchId
  ...
  const dashFilteredBranches = currentUser?.role === 'branch_admin' && currentUser.branchId
    ? branches.filter(b => b.roundId === dashSelectedRoundId && b.id === currentUser.branchId)
          ...
          {authView === 'login' && (
            <>
              <div className="text-center mb-6">
                ...
                <p className="text-[var(--text-secondary)] text-sm mt-1">تکایە زانیارییەکانی بەکارهێنەر بنووسە</p>
              </div>
    ...
    <div className="min-h-screen flex flex-col bg-[var(--bg-main)] text-[var(--text-primary)] transition-colors duration-200" dir="rtl">
      <header className="h-16 bg-[var(--bg-card)] border-b border-[var(--border-color)] px-6 flex items-center justify-between shadow-md">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-base font-bold">سیستەمی شیکاری ئەنجامەکانی هەڵبژاردن</h1>
            <p className="text-xs text-[var(--text-secondary)]">بەکارهێنەر: <span className="font-bold text-blue-500">{currentUser.name}</span> ({currentUser.role === 'super_admin' ? 'بەڕێوەبەری گشتی' : currentUser.role === 'branch_admin' ? 'ئەدمینی لق' : 'چاودێر'})</p>
          </div>
        </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        <aside className="w-64 bg-[var(--bg-card)] border-l border-[var(--border-color)] p-4 flex flex-col gap-2 shadow-sm">
          ...
          {currentUser.role === 'super_admin' && (
            ...
            >
              <span>پەنێڵی کۆنتڕۆڵ و ئەکاونتەکان</span>
              <span>⚙️</span>
            </button>
          )}
        </aside>
        ...
        <main className="flex-1 p-6 overflow-y-auto space-y-6">
              ...
              <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-6 shadow-sm space-y-6">
                      ...
                      rounds.map(r => (
                        <button
                          ...
                          onClick={() => {
                            ...
                            if (currentUser.role !== 'branch_admin') setDashSelectedBranch('all');
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
                  <div>
                    ...
                    >
                      {currentUser.role !== 'branch_admin' && <option value="all">هەموو لقەکان گشتی</option>}
          ...
          {activeMainTab === 'controlPanel' && currentUser.role === 'super_admin' && (
            <div className="space-y-6">
              <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-6 shadow-sm space-y-6">
                <div className="border-b border-[var(--border-color)] pb-3 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-[var(--text-secondary)]">دروستکردنی ئەکاونتی نوێ بۆ لقەکان یان چاودێران</p>
                  </div>
                </div>
                ...
                <form onSubmit={handleCreateAccount} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    >
                      ...
                      <option value="viewer">چاودێر (Viewer - تەنها بینین)</option>
                    </select>
                  </div>

                  {newAccRole === 'branch_admin' && (
                    <div>
              ...
              <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-6 shadow-sm space-y-4">
                <h3 className="text-lg font-bold">لیستی هەموو ئەکاونتە تۆمارکراوەکان</h3>
                <div className="overflow-x-auto">
                  <table className="w-full text-right border-collapse">
                    <thead>
                      <tr className="border-b border-[var(--border-color)] text-[var(--text-secondary)] text-sm">
