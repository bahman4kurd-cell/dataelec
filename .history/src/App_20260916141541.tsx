...
// فەرهەنگی زمانەکان بۆ گۆڕینی ڕووکارەکە
const translations = {
  ckb: {
...
export function App() {
  ...
  if (!isLoggedIn || !currentUser) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[var(--bg-main)] text-[var(--text-primary)] p-4" dir={lang === 'ckb' ? 'rtl' : 'ltr'}>
        <div className="bg-[var(--bg-card)] border border-[var(--border-color)] shadow-2xl rounded-2xl p-8 w-full max-w-md relative">
          
          {/* دوگمەی گۆڕینی زمان لە لۆگیندا */}
          <div className="absolute top-4 left-4">
            ...
            >
              ...
              <span>{lang === 'ckb' ? 'English' : 'کوردی'}</span>
            </button>
          </div>
              ...
              <form onSubmit={handleLogin} className="space-y-4">
                <div>
                  ...
                  <div className="relative">
                    <User className={`absolute ${lang === 'ckb' ? 'right-3' : 'left-3'} top-3 w-5 h-5 text-gray-400`} />
                    <input 
                      type="text" 
                      value={username}
                      onChange={(e) => setUsername(e.target.value)}
                      className={`w-full bg-[var(--bg-main)] border border-[var(--border-color)] rounded-xl ${lang === 'ckb' ? 'pr-10 pl-4' : 'pl-10 pr-4'} py-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none`}
                      placeholder="admin"
                      required 
                    />
                  </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    <Lock className={`absolute ${lang === 'ckb' ? 'right-3' : 'left-3'} top-3 w-5 h-5 text-gray-400`} />
                    <input 
                      type="password" 
                      value={password}
            ...
            <>
              ...
              <form onSubmit={handleForgotPassword} className="space-y-4">
                <div>
                  ...
                  <div className="relative">
                    <Mail className={`absolute ${lang === 'ckb' ? 'right-3' : 'left-3'} top-3 w-5 h-5 text-gray-400`} />
                    <input 
                      type="email" 
                      value={recoveryEmail}
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[var(--bg-main)] text-[var(--text-primary)] transition-colors duration-200" dir={lang === 'ckb' ? 'rtl' : 'ltr'}>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                      {dashboardData.map((item, idx) => (
                        <div key={item.partyName} className="relative bg-[var(--bg-card)] border-2 border-dashed border-[var(--border-color)] p-5 rounded-2xl flex flex-col items-center text-center gap-3 shadow-sm">
                          <div className="absolute -top-3 start-4 px-3 py-0.5 text-xs font-bold text-white rounded-full shadow" style={{ backgroundColor: item.hexColor }}>
                            گرێی #{idx + 1}
                          </div>
