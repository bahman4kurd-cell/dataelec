...
type LanguageType = 'ckb' | 'en' | 'ar' | 'fa';
...
// فەرهەنگی زمانەکان بۆ گۆڕینی ڕووکارەکە (زیادکردنی عەرەبی و فارسی)
const translations = {
  ckb: {
...
export function App() {
  ...
  // گۆڕینی سووڕی زمانەکان (کوردی -> ئینگلیزی -> عەرەبی -> فارسی)
  const toggleLanguage = () => {
    setLang(prev => {
      if (prev === 'ckb') return 'en';
  ...
  // دیاریکردنی ئاراستەی نوسین بەپێی زمانەکە (LTR بۆ ئینگلیزی، RTL بۆ ئەوانی تر)
  const getDir = () => (lang === 'en' ? 'ltr' : 'rtl');

  // زانیارییە وەرگیراوەکانی کودی HTMLـەکەی تۆ (بەبێ دەستکاری و فۆرماتی ڕەسەن)
  const htmlStaticSnippet = (
    <div className="hidden" aria-hidden="true">
      {/* Sidebar / Navigation */}
      <aside id="sidebar" className="w-64 bg-white border-l border-gray-200 flex flex-col transition-all duration-300">
        <div className="p-4 border-b border-gray-100 flex items-center justify-between">
          <span id="navHeader" className="font-bold text-gray-600 text-sm">بەشە سەرەکییەکان</span>
        </div>
        <nav className="p-4 space-y-2 flex-1">
          <a href="#" className="flex items-center gap-3 px-4 py-2.5 bg-blue-600 text-white rounded-lg shadow-sm">
            <span id="menuDashboard">داشبۆرد و نمودارەکان</span>
          </a>
          <a href="#" className="flex items-center gap-3 px-4 py-2.5 text-gray-600 hover:bg-gray-100 rounded-lg">
            <span id="menuRounds">دۆرە هەڵبژاردنەکان</span>
          </a>
          <a href="#" className="flex items-center gap-3 px-4 py-2.5 text-gray-600 hover:bg-gray-100 rounded-lg">
            <span id="menuControl">کۆنتڕۆڵ پەنڵ و حسابات</span>
          </a>
        </nav>
      </aside>
      ...
      <header className="bg-white border-b border-gray-200 h-16 flex items-center justify-between px-6">
        <div className="flex items-center gap-4">
          <h1 id="systemTitle" className="font-bold text-lg text-blue-900">سیستەمی شیکاری ئەنجامی هەڵبژاردن</h1>
        </div>
        <div className="flex items-center gap-4">
          <select id="languageSelect" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg p-2">
            <option value="ku">کوردی (سۆرانی)</option>
            <option value="kmr">کوردی (کرمانجی)</option>
            <option value="ar">عربي</option>
          </select>
          <button id="logoutBtn" className="bg-rose-50 text-rose-600 border border-rose-200 px-4 py-1.5 rounded-lg text-sm font-medium">چوونەدەرەوە</button>
        </div>
      </header>
      ...
      <main className="p-6 space-y-6">
        <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm flex justify-between items-center">
          <div>
            <h2 id="mainHeading" className="text-xl font-bold text-gray-800">داشبۆرد و شیکاری ئەنجامەکان</h2>
            <p id="subHeading" className="text-gray-500 text-sm mt-1">خولی هەڵبژاردراوە کە: پەڕڵەمانی کوردستان 2024</p>
          </div>
          <div className="flex items-center gap-2">
            <span id="chartTypeText" className="text-sm text-gray-600">جۆری شێوازی چارت:</span>
            <span id="donutChartText" className="bg-gray-100 px-3 py-1 rounded-md text-sm font-medium">هێڵکاری دۆنات (Donut Chart)</span>
          </div>
        </div>

        <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm space-y-4">
          <p id="selectRoundText" className="text-sm font-semibold text-gray-700">دیاریکردنی خولی هەڵبژاردن بۆ بینینی چارتەکان:</p>
          <div>
            <button id="parliamentBtnText" className="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-medium shadow-sm">پەڕڵەمانی کوردستان 2024</button>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4 border-t border-gray-100">
            <div>
              <label id="filterRegionText" className="block text-xs font-medium text-gray-500 mb-1">فلتەر بەپێی لقی هەڵبژاردن</label>
              <select className="w-full bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg p-2.5">
                <option id="allBranchesText">هەموو لقیەکان گشتی</option>
              </select>
            </div>
            <div>
              <label id="filterAreaText" className="block text-xs font-medium text-gray-500 mb-1">فلتەر بەپێی ناوچە / بنکە</label>
              <select className="w-full bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg p-2.5">
                <option id="allAreasText">هەموو ناوچەکانی ئەم لقە</option>
              </select>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            <h3 id="chartResultsHeading" className="text-lg font-bold text-gray-800">ئەنجامی چارتەکان بۆ خولی: پەڕڵەمانی کوردستان 2024</h3>
            <div id="totalVotesText" className="bg-blue-50 text-blue-700 px-4 py-2 rounded-lg text-sm font-bold border border-blue-100">کۆی دەنگەکان: 13,732</div>
          </div>
        </div>
      </main>
    </div>
        ...
        <div className="bg-[var(--bg-card)] border border-[var(--border-color)] shadow-2xl rounded-2xl p-8 w-full max-w-md relative">
          ...
          <div className="absolute top-4 left-4">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 bg-[var(--bg-main)] hover:bg-[var(--bg-hover)] border border-[var(--border-color)] rounded-xl text-xs font-semibold transition-all shadow-sm"
              title="Change Language / گۆڕینی زمان"
            >
              <Globe className="w-4 h-4 text-blue-500" />
              <span>
                {lang === 'ckb' ? 'English' : lang === 'en' ? 'عربي' : lang === 'ar' ? 'فارسی' : 'کوردی'}
              </span>
            </button>
          </div>
    ...
    <div className="min-h-screen flex flex-col bg-[var(--bg-main)] text-[var(--text-primary)] transition-colors duration-200" dir={getDir()}>
      ...
      <header className="h-16 bg-[var(--bg-card)] border-b border-[var(--border-color)] px-6 flex items-center justify-between shadow-md">
        ...
        <div className="flex items-center gap-4">
          <button
            onClick={toggleLanguage}
            className="flex items-center gap-2 bg-[var(--bg-main)] hover:bg-[var(--bg-hover)] border border-[var(--border-color)] px-3 py-1.5 rounded-lg text-sm font-semibold transition shadow-sm"
              ...
              <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-6 shadow-sm space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
                  <div>
                    ...
                    >
                      {currentUser.role !== 'branch_admin' && currentUser.role !== 'viewer' && <option value="all">هەموو لقەکان گشتی</option>}
                  ...
                  <table className="w-full text-start border-collapse">
                    <thead>
                      <tr className="border-b border-[var(--border-color)] text-[var(--text-secondary)] text-sm">
                        ...
                        {currentUser.role === 'super_admin' && <th className="py-3 px-4">کردارەکان</th>}
                      </tr>
                    </thead>
                    ...
                    <div className="mt-6 pt-6 border-t border-[var(--border-color)] space-y-4">
                      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                        <div>
                          ...
                          <h3 className="text-lg font-bold mt-2">ناوچەکانی سەر بە لق</h3>
                        </div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {currentRegions.length === 0 ? (
                          <p className="text-sm text-amber-500 font-medium">⚠️ هیچ ناوچەیەک بۆ ئەم لقە نییە، دەتوانیت دەنگەکان ڕاستەوخۆ لێرەوە داخڵ بکەیت:</p>
                        ) : (
                        ...
                        <div className="mt-6 bg-[var(--bg-main)] border-2 border-blue-500/40 rounded-xl p-5 space-y-4 shadow-sm">
                          <div className="flex justify-between items-center">
                            <h4 className="font-bold text-base text-blue-600">
                              📋 داخڵکردنی دەنگی لایەنەکان ڕاستەوخۆ لەسەر ئاستی لق
                            </h4>
                ...
                <>
                  <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-6 shadow-sm space-y-6">
                    <div className="border-b border-[var(--border-color)] pb-3 flex items-center gap-3">
                      ...
                      <div>
                        <h2 className="text-xl font-bold">پەنێڵی بەڕێوەبردنی ئەکاونتەکان</h2>
                        <p className="text-sm text-[var(--text-secondary)]">دروستکردنی ئەکاونتی نوێ بۆ لقەکان یان چاودێران (Viewer)</p>
                      </div>
                    </div>
                    ...
                    <form onSubmit={handleCreateAccount} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      <div>
                        <label className="block text-sm font-medium mb-1 text-[var(--text-secondary)]">ناوی تەواو / ناوەند</label>
                        <input
                          type="text"
                          value={newAccName}
                      ...
                      <div>
                        <label className="block text-sm font-medium mb-1 text-[var(--text-secondary)]">ناوی بەکارهێنەر (Username)</label>
                        <input
                          type="text"
                      ...
                      <div>
                        <label className="block text-sm font-medium mb-1 text-[var(--text-secondary)]">وشەی نهێنی (Password)</label>
                        <input
                          type="text"
                      ...
                      <div>
                        <label className="block text-sm font-medium mb-1 text-[var(--text-secondary)]">ڕۆڵی بەکارهێنەر</label>
                        <select
                          value={newAccRole}
                          onChange={(e) => setNewAccRole(e.target.value as UserRole)}
                          className="w-full bg-[var(--bg-main)] border border-[var(--border-color)] rounded-lg px-3 py-2 text-sm"
                        >
                          <option value="branch_admin">ئەدمینی لق (Branch Admin)</option>
                          <option value="viewer">چاودێر (Viewer - بینین)</option>
                        </select>
                      </div>
                      ...
                      {(newAccRole === 'branch_admin' || newAccRole === 'viewer') && (
                        <div>
                          ...
                          <select
                            ...
                            required
                          >
                            <option value="">-- لقی مەبەست هەڵبژێرە --</option>
                            {branches.map(b => (
                              <option key={b.id} value={b.id}>{b.name}</option>
                            ))}
                          </select>
                  ...
                  <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-6 shadow-sm space-y-4">
                    <h3 className="text-lg font-bold">لیستی هەموو ئەکاونتەکان و بینین/گۆڕینی پاسوۆردەکانیان لە داتابەیسدا</h3>
                    <div className="overflow-x-auto">
                      <table className="w-full text-start border-collapse">
                        <thead>
                          <tr className="border-b border-[var(--border-color)] text-[var(--text-secondary)] text-sm">
                            <th className="py-3 px-4">ناوی بەکارهێنەر</th>
                            <th className="py-3 px-4">یوزەرنەیم</th>
                            <th className="py-3 px-4">پاسوۆرد (داتابەیس)</th>
                            <th className="py-3 px-4">ڕۆڵ</th>
                            <th className="py-3 px-4">لقی پەیوەندیدار</th>
                              ...
                              <tr key={u.id} className="hover:bg-[var(--bg-hover)]">
                                ...
                                <td className="py-3 px-4">
                                  <span className={`px-2.5 py-1 rounded-full text-xs font-bold ${
                                    ...
                                    {u.role === 'super_admin' ? 'بەڕێوەبەری گشتی' : u.role === 'branch_admin' ? 'ئەدمینی لق' : 'چاودێر'}
                                  </span>
                                </td>
                                <td className="py-3 px-4">{targetBranch ? targetBranch.name : 'هیچ / گشتی'}</td>
                                <td className="py-3 px-4">
                                  {u.id !== 1 && (
                                    <button
              ...
              {currentUser.role === 'branch_admin' && (
                <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-6 shadow-sm space-y-4 max-w-xl mx-auto">
                  <div className="flex items-center gap-3 border-b border-[var(--border-color)] pb-3">
                    ...
                    <div>
                      <h2 className="text-xl font-bold">گۆڕینی وشەی نهێنی (پاسوۆردی خۆت)</h2>
                      <p className="text-sm text-[var(--text-secondary)]">ئەدمینی لق دەتوانێت پاسوۆردەکەی لێرە بگۆڕێت و لە داتابەیسدا نوێ دەبێتەوە</p>
                    </div>
                  </div>
                  ...
                  <form onSubmit={handleUpdateMyPassword} className="space-y-4">
                    <div>
                      <label className="block text-sm font-medium mb-1 text-[var(--text-secondary)]">ناوی بەکارهێنەرەکەت</label>
                      <input
                        type="text"
                    ...
                    <div>
                      <label className="block text-sm font-medium mb-1 text-[var(--text-secondary)]">وشەی نهێنی نوێ</label>
                      <input
                        type="text"
                    ...
                    >
                      پاشەکەوتکردنی پاسوۆردی نوێ لە داتابەیسدا
                    </button>
                  </form>
                </div>
              )}
