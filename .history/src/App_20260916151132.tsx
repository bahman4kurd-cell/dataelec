...
const translations = {
  ckb: {
    ...
    filterBranchLabel: 'فلتەر بەپێی لقی هەڵبژاردن (مۆڵتی چۆیس)',
    allBranchesOption: 'هەموو لقەکان گشتی',
    filterAreaLabel: 'فلتەر بەپێی ناوچە / بنکە (مۆڵتی چۆیس)',
    allAreasOption: 'هەموু ناوچەکانی لقە هەڵبژاردراوەکان',
    filterPartyLabel: 'فلتەری دەنگی لایەنەکان (چێک بۆکس)',
  ...
  kmr: {
    ...
    filterPartyLabel: 'فلتەرا دەنگێن لایەنان (چێک بۆکس)',
  ...
  en: {
    ...
    filterPartyLabel: 'Filter Parties (Checkbox)',
  ...
  ar: {
    ...
    filterPartyLabel: 'تصفية الأحزاب (صندوق اختيار)',
  ...
  fa: {
    ...
    filterPartyLabel: 'فیلتر احزاب (چک باکس)',
...
export function App() {
  ...
  useEffect(() => {
    if (dashSelectedRoundId) {
      const roundBranches = currentUser?.role === 'branch_admin' && currentUser.branchId 
  ...
  // شیکارکردن و هێنانی داتای دەنگەکان بۆ داشبۆرد بەپێی فلتەری Multi-select
  ...
  // شیکارکردنی داتا بۆ تابی ڕاپۆرتەکان بەپێی فلتەری تایبەتی خۆی
  ...
  // فەنکشن بۆ پرنتکردن و داونلۆدی PDF بەبێ تێکچوونی زمانی کوردی
    ...
    <div className="min-h-screen flex flex-col bg-[var(--bg-main)] text-[var(--text-primary)] transition-colors duration-200" dir={getDir()}>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="flex-1 p-6 overflow-y-auto space-y-6">
            ...
            <div className="space-y-6">
              <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-6 shadow-sm space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2">
                  ...
                  {/* فلتەری لقەکان بە شێوازی Multi-select Checkbox */}
                  <div className="relative">
                  ...
                  {/* فلتەری ناوچەکان بە شێوازی Multi-select Checkbox */}
                  <div className="relative">
                  ...
                  {/* فلتەری لایەنەکان بە شێوازی Checkbox */}
                  <div className="relative">
                                ...
                                <td className="py-3 px-4">
                                  <div className="flex items-center gap-2">
                                    <input
                                      ...
                                      id={`pass_input_${u.id}`}
                                      className="bg-[var(--bg-main)] border border-[var(--border-color)] rounded px-2 py-1 text-xs w-28"
                                    />
                                    <button
                                      onClick={() => {
                                        const inputEl = document.getElementById(`pass_input_${u.id}`) as HTMLInputElement;
                                        if (inputEl) handleAdminUpdateUserPassword(u.id, inputEl.value);
                                      }}
              ...
              {currentUser.role === 'branch_admin' && (
                <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-6 shadow-sm space-y-6 max-w-xl mx-auto">
                  ...
                  <form onSubmit={handleUpdateMyPassword} className="space-y-4">
                    ...
                    <button
                      type="submit"
                      className="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2.5 rounded-lg text-sm transition shadow"
                    >
          ...
          {/* تابی نوێی ڕاپۆرتەکان (Reports & Print & PDF) */}
          {activeMainTab === 'reports' && (
            <div className="space-y-6">
              <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-6 shadow-sm space-y-6 print:hidden">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-[var(--border-color)] pb-4">
                  ...
                  <div className="flex items-center gap-3">
                    <button
                      ...
                      className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm font-bold shadow transition"
                    >
                    ...
                    <button
                      ...
                      className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-lg text-sm font-bold shadow transition"
                    >
                ...
                {/* خولەکانی هەڵبژاردن بۆ فلتەری ڕاپۆرت */}
                <div className="space-y-3">
                  <span className="text-xs font-bold text-[var(--text-secondary)]">{t.selectRoundPrompt}</span>
                  <div className="flex flex-wrap gap-2">
                    {rounds.length === 0 ? (
                ...
                {/* فلتەرەکانی لق و ناوچە بە شێوازی مۆڵتی چۆیس (Multi-choice) */}
              ...
              {/* ناوەڕۆکی ڕاپۆرتی فەرمی کە چارت و لایەنەکانیش لەخۆدەگرێت بۆ پرنت و PDF */}
              <div ref={reportRef} className="bg-white text-slate-900 border border-[var(--border-color)] rounded-xl p-8 shadow-md space-y-8 print:shadow-none print:border-none print:p-0">
                <div className="text-center border-b-2 border-slate-200 pb-6 space-y-2">
                  <h2 className="text-2xl font-black text-slate-900">ڕاپۆرتی فەرمی ئەنجامەکانی هەڵبژاردن</h2>
                  <p className="text-sm font-bold text-blue-600">{repSelectedRoundObj?.name || 'خولی هەڵبژاردن'}</p>
                  <p className="text-xs text-slate-500">بەرواری دەرکردنی ڕاپۆرت: {new Date().toLocaleDateString('en-GB')}</p>
                </div>

                <div className="grid grid-cols-2 gap-4 text-xs font-semibold bg-slate-50 p-4 rounded-xl border border-slate-200">
                  <div>کۆی گشتی دەنگەکان: <span className="font-bold text-blue-600">{totalReportVotes.toLocaleString()}</span></div>
                  <div>جۆری هەڵبژاردن: <span className="font-bold">{repSelectedRoundObj?.type || '-'}</span></div>
                </div>

                {/* هێنانی چارت و شێوازی گرافیکی لەگەڵ ڕاپۆرتەکە */}
                <div className="flex flex-col items-center justify-center py-4 border-b border-slate-200 pb-6">
                  <h4 className="font-bold text-sm text-slate-700 mb-4">هێڵکاری بازنەیی ڕێژەی دەنگەکان</h4>
                  <div className="relative w-64 h-64 flex items-center justify-center">
                    <svg viewBox="0 0 100 100" className="w-full h-full drop-shadow">
                      {createSvgSlices(reportData, true)}
                    </svg>
                    <div className="absolute inset-0 flex flex-col items-center justify-center text-center pointer-events-none p-2">
                      <span className="text-[10px] font-semibold text-slate-500">کۆی گشتی</span>
                      <span className="text-xs font-extrabold text-slate-800">{totalReportVotes.toLocaleString()}</span>
                    </div>
                  </div>
                </div>

                {/* خشتەی وردەکاریی دەنگی لایەنەکان */}
                <div className="space-y-4">
                  <h3 className="text-lg font-bold text-slate-800">لیستی وردەکاری دەنگ و ڕێژەی لایەنە سیاسییەکان</h3>
                  <div className="overflow-x-auto">
                    <table className="w-full text-start border-collapse border border-slate-300">
                      <thead>
                        <tr className="bg-slate-100 text-slate-700 text-xs font-bold border-b border-slate-300">
                          <th className="py-2.5 px-3 border-e border-slate-300">#</th>
                          <th className="py-2.5 px-3 border-e border-slate-300">{t.partyNameHeader}</th>
                          <th className="py-2.5 px-3 border-e border-slate-300">ژمارەی دەنگەکان</th>
                          <th className="py-2.5 px-3">ڕێژەی سەدی (%)</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-200 text-xs">
                        {reportData.map((item, index) => (
                          <tr key={item.partyId} className="hover:bg-slate-50">
                            <td className="py-2.5 px-3 border-e border-slate-200 font-bold">{index + 1}</td>
                            <td className="py-2.5 px-3 border-e border-slate-200 font-semibold flex items-center gap-2">
                              <span className="w-3 h-3 rounded-full border border-slate-300" style={{ backgroundColor: item.hexColor }}></span>
                              <span>{item.partyName}</span>
                            </td>
                            <td className="py-2.5 px-3 border-e border-slate-200 font-bold">{item.votes.toLocaleString()}</td>
                            <td className="py-2.5 px-3 font-extrabold text-blue-600">{item.percentage}%</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                ...
                <div className="pt-8 flex justify-between text-xs text-slate-500 border-t border-slate-200">
                  <div>واژووی بەرپرسی لێژنەی باڵای هەڵبژاردن</div>
                  <div>مۆری فەرمی سیستەم</div>
                </div>
              </div>
            </div>
          )}
