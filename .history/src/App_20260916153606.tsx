...
export function App() {
  ...
  // بەکارهێنانی فەنکشنی html2pdf وەک داواکراو بۆ داونلۆدی PDF
        ...
        <main className="flex-1 p-6 overflow-y-auto space-y-6">
                  ...
                  <form onSubmit={handleUpdateMyPassword} className="space-y-4">
                    <div>
                      ...
                      <input 
                        ...
                        disabled 
                        className="w-full bg-[var(--bg-main)] border border-[var(--border-color)] rounded-lg px-3 py-2 text-sm opacity-60 font-mono"
                      />
                    </div>
          ...
          {activeMainTab === 'reports' && (
            <div className="space-y-6">
              ...
              {/* بەستنەوەی ڕێفرنس (reportRef) بەو بەشەی کە دەکرێت بە PDF */}
              <div ref={reportRef} className="bg-white text-gray-900 border border-gray-200 rounded-xl p-8 shadow-md space-y-6">
                <div className="text-center border-b border-gray-200 pb-6 space-y-2">
                  ...
                  <h2 className="text-2xl font-black text-gray-900">ڕاپۆرتی فەرمی ئەنجامەکانی هەڵبژاردن</h2>
                  <p className="text-sm font-semibold text-gray-600">{repSelectedRoundObj?.name || 'خولی هەڵبژاردن'}</p>
                  <p className="text-xs text-gray-500">بەرواری دەرچوون: {new Date().toLocaleDateString('en-GB')}</p>
                </div>

                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 bg-gray-50 p-4 rounded-xl border border-gray-200 text-xs font-bold text-gray-700">
                  <div>جۆری هەڵبژاردن: <span className="text-emerald-700">{repSelectedRoundObj?.type || 'گشتی'}</span></div>
                  <div>کۆی دەنگدەران: <span className="text-emerald-700">{repSelectedRoundObj?.totalVoters?.toLocaleString() || '0'}</span></div>
                  <div>کۆی دەنگە دراوەکان: <span className="text-emerald-700">{totalReportVotes.toLocaleString()}</span></div>
                  <div>ژمارەی لقەکان: <span className="text-emerald-700">{repSelectedBranchIds.length}</span></div>
                </div>

                <div className="overflow-x-auto pt-2">
                  <table className="w-full text-start border-collapse border border-gray-200 text-sm">
                    <thead>
                      <tr className="bg-gray-100 text-gray-800 border-b border-gray-200">
                        <th className="py-3 px-4 border-e border-gray-200">#</th>
                        <th className="py-3 px-4 border-e border-gray-200">{t.partyNameHeader}</th>
                        <th className="py-3 px-4 border-e border-gray-200">{t.votesCountLabel}</th>
                    ...
                    <tbody className="divide-y divide-gray-200">
                      {reportData.map((item, index) => (
                        <tr key={item.partyId} className="hover:bg-gray-50">
                          <td className="py-3 px-4 border-e border-gray-200 font-bold">{index + 1}</td>
                          <td className="py-3 px-4 border-e border-gray-200 font-semibold flex items-center gap-2">
                            <span className="w-3.5 h-3.5 rounded-full border border-gray-300" style={{ backgroundColor: item.hexColor }}></span>
                            <span>{item.partyName}</span>
                          </td>
                          <td className="py-3 px-4 border-e border-gray-200 font-mono font-bold">{item.votes.toLocaleString()}</td>
                          <td className="py-3 px-4 font-mono font-bold text-emerald-700">{item.percentage}%</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                <div className="pt-8 flex justify-between items-end border-t border-gray-200 text-xs text-gray-600 font-bold">
                  <div>واژۆی بەرپرسی هۆبەی تەکنیک و ئامار</div>
                  <div>واژوی بەرپرسی بەشی هەڵبژاردن</div>
                </div>
              </div>
            </div>
          )}
