        ...
        <main className="flex-1 p-6 overflow-y-auto space-y-6">
                          ...
                          {users.map(u => {
                            ...
                            return (
                              <tr key={u.id} className="hover:bg-[var(--bg-hover)]">
                                ...
                                <td className="py-3 px-4">
                                  <div className="flex items-center gap-2">
                                    <input 
                                      ...
                                      id={`pass_input_${u.id}`}
                                      className="bg-[var(--bg-main)] border border-[var(--border-color)] rounded px-2 py-1 text-xs w-24 font-mono"
                                    />
                                    <button 
                                      onClick={() => {
                                ...
                                <td className="py-3 px-4 text-[var(--text-secondary)]">{bObj ? bObj.name : t.generalOrNone}</td>
                                <td className="py-3 px-4">
                                  {u.id !== 1 && (
          ...
          {activeMainTab === 'reports' && currentUser.role !== 'viewer' && (
            <div className="space-y-6">
                            ...
                            <div 
                              ...
                              onClick={() => {
                                if (isChecked) {
                                  ...
                                  const bRegIds = regions.filter(reg => reg.branchId === branch.id).map(r => r.id);
                                  setRepSelectedRegionIds(repSelectedRegionIds.filter(id => !bRegIds.includes(id)));
                                } else {
              ...
              {/* بەشی پاوەڕۆنت و کاغەزی ڕاپۆرت بۆ PDF */}
              <div ref={reportRef} className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-8 shadow-sm space-y-8 print:border-none print:shadow-none print:p-2">
                <div className="text-center border-b border-[var(--border-color)] pb-6 space-y-2">
                  <div className="inline-flex p-3 bg-emerald-600/10 text-emerald-600 rounded-full mb-1">
                    <FileText className="w-8 h-8" />
                  </div>
                  <h2 className="text-2xl font-extrabold">{t.systemTitle}</h2>
                  <p className="text-sm font-bold text-[var(--text-secondary)]">ڕاپۆرتی فەرمی ئەنجامەکان - {repSelectedRoundObj?.name}</p>
                  <div className="flex justify-center gap-6 text-xs text-[var(--text-secondary)] pt-2">
                    <span>بەروار: {repSelectedRoundObj?.date || 'نەدیاریکراو'}</span>
                    <span>جۆر: {repSelectedRoundObj?.type || 'نەدیاریکراو'}</span>
                    <span>{t.totalVotesLabel} <strong className="text-emerald-600">{totalReportVotes.toLocaleString()}</strong></span>
                  </div>
                </div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-start border-collapse border border-[var(--border-color)]">
                    ...
                    <tbody className="divide-y divide-[var(--border-color)] text-sm font-semibold">
                      {reportData.map((item, idx) => (
                        <tr key={item.partyId} className="hover:bg-[var(--bg-hover)]">
                          <td className="py-3 px-4 border-e border-[var(--border-color)]">{idx + 1}</td>
                          <td className="py-3 px-4 border-e border-[var(--border-color)] flex items-center gap-2">
                            <span className="w-3.5 h-3.5 rounded-full border border-gray-300" style={{ backgroundColor: item.hexColor }}></span>
                            <span>{item.partyName}</span>
                          </td>
                          <td className="py-3 px-4 border-e border-[var(--border-color)] font-mono">{item.votes.toLocaleString()}</td>
                          <td className="py-3 px-4">
                            <div className="flex items-center gap-2">
                              <span>{item.percentage}%</span>
                              <div className="flex-1 bg-[var(--bg-main)] h-2 rounded-full overflow-hidden border border-[var(--border-color)] max-w-[120px]">
                                <div className="h-full" style={{ width: `${item.percentage}%`, backgroundColor: item.hexColor }}></div>
                              </div>
                            </div>
                          </td>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 pt-6 border-t border-[var(--border-color)] text-center text-xs text-[var(--text-secondary)]">
                  <div className="p-4 bg-[var(--bg-main)] border border-[var(--border-color)] rounded-xl">
                    <p className="font-bold text-[var(--text-primary)]">بەرپرسی سیستم / بەڕێوەبەر</p>
                    <p className="mt-8 border-t border-dashed border-gray-400 pt-1">واژوو</p>
                  </div>
                  <div className="p-4 bg-[var(--bg-main)] border border-[var(--border-color)] rounded-xl">
                    <p className="font-bold text-[var(--text-primary)]">لیژنەی باڵای هەڵبژاردن</p>
                    <p className="mt-8 border-t border-dashed border-gray-400 pt-1">واژوو</p>
                  </div>
                  <div className="p-4 bg-[var(--bg-main)] border border-[var(--border-color)] rounded-xl col-span-2 sm:col-span-1">
                    <p className="font-bold text-[var(--text-primary)]">مۆری فەرمی</p>
                    <p className="mt-8 border-t border-dashed border-gray-400 pt-1">مۆر</p>
                  </div>
                </div>
              </div>
            </div>
