        ...
        <main className="flex-1 p-6 overflow-y-auto space-y-6">
          ...
          {activeMainTab === 'controlPanel' && (
            <div className="space-y-6">
              ...
              {currentUser.role === 'branch_admin' && (
                <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-6 shadow-sm max-w-xl mx-auto space-y-6">
                  <div className="border-b border-[var(--border-color)] pb-3 flex items-center gap-3">
                  ...
                  <form onSubmit={handleUpdateMyPassword} className="space-y-4">
                    <div>
                      ...
                      <input
                        ...
                        placeholder="پاسوۆردی نوێ لێرە بنووسە..."
                        className="w-full bg-[var(--bg-main)] border border-[var(--border-color)] rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500"
                        required
                      />
                    </div>
          ...
          {activeMainTab === 'reports' && currentUser.role !== 'viewer' && (
            <div className="space-y-6">
              ...
              {/* بەشی پەرپەڕی فەرمی کە بۆ پرنت و PDF بەکاردێت */}
              <div ref={reportRef} className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-8 shadow-sm space-y-6 print:border-none print:shadow-none print:p-2">
                <div className="text-center space-y-2 border-b border-[var(--border-color)] pb-6">
                  ...
                  <p className="text-sm font-bold text-emerald-600">ڕاپۆرتی فەرمی ئەنجامەکانی هەڵبژاردن</p>
                  <p className="text-xs text-[var(--text-secondary)]">
                    خولی هەڵبژاردن: <span className="font-bold">{repSelectedRoundObj?.name || 'دیارنەکراو'}</span> | ڕێکەوتی دەرچواندن: {new Date().toLocaleDateString()}
                  </p>
                </div>

                <div className="flex justify-between items-center bg-[var(--bg-main)] border border-[var(--border-color)] px-6 py-4 rounded-xl">
                  <span className="font-bold text-sm">کۆی گشتی دەنگەکان لەم فلتەرەدا:</span>
                  <span className="font-extrabold text-emerald-600 text-lg">{totalReportVotes.toLocaleString()} دەنگ</span>
                </div>

                <div className="overflow-x-auto">
                  <table className="w-full text-start border-collapse">
                    <thead>
                      <tr className="border-b-2 border-[var(--border-color)] text-[var(--text-secondary)] text-sm">
                        <th className="py-3 px-4">#</th>
                        <th className="py-3 px-4">{t.partyNameHeader}</th>
                        <th className="py-3 px-4 text-center">{t.votesCountLabel}</th>
                        <th className="py-3 px-4 text-end">{t.percentageLabel}</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-[var(--border-color)] text-sm">
                      {reportData.map((item, idx) => (
                        <tr key={item.partyName} className="hover:bg-[var(--bg-hover)]">
                          <td className="py-3 px-4 text-[var(--text-secondary)] font-mono">{idx + 1}</td>
                          <td className="py-3 px-4 font-bold flex items-center gap-2.5">
                            <span className="w-3.5 h-3.5 rounded-full border border-gray-300" style={{ backgroundColor: item.hexColor }}></span>
                            <span>{item.partyName}</span>
                          </td>
                          <td className="py-3 px-4 text-center font-mono font-bold">{item.votes.toLocaleString()}</td>
                          <td className="py-3 px-4 text-end font-mono font-bold text-emerald-600">{item.percentage}%</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                <div className="pt-12 grid grid-cols-2 text-center text-xs font-bold text-[var(--text-secondary)] border-t border-[var(--border-color)] mt-12">
                  <div>
                    <p>واژۆی سەرپەرشتیاری هەڵبژاردن</p>
                    <div className="h-12"></div>
                    <p className="border-t border-[var(--border-color)] inline-block px-12 pt-1">........................................</p>
                  </div>
                  <div>
                    <p>مۆری فەرمی ناوەند / لق</p>
                    <div className="h-12"></div>
                    <p className="border-t border-[var(--border-color)] inline-block px-12 pt-1">........................................</p>
                  </div>
                </div>
