...
export function App() {
  ...
  const getFilteredReportVotes = () => {
    ...
    repSelectedBranchIds.forEach(bId => {
              ...
              <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-6 shadow-sm space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2">
                  ...
                  <div className="relative">
                    ...
                    <div 
                      onClick={() => {
                        if (dashSelectedBranchIds.length > 0) {
                          setIsRegionDropdownOpen(!isRegionDropdownOpen);
                          setIsBranchDropdownOpen(false);
                          setIsPartyDropdownOpen(false);
                        }
                      }}
                      className={`w-full bg-[var(--bg-main)] border border-[var(--border-color)] rounded-lg px-3 py-2 text-sm flex items-center justify-between select-none ${dashSelectedBranchIds.length === 0 ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
                    >
                      <span className="truncate">
                    ...
                    {isRegionDropdownOpen && dashSelectedBranchIds.length > 0 && (
                      <div className="absolute z-20 mt-1 w-full bg-[var(--bg-card)] border border-[var(--border-color)] rounded-lg shadow-xl p-3 space-y-2 max-h-60 overflow-y-auto">
                        <div className="flex items-center justify-between border-b border-[var(--border-color)] pb-2 mb-2">
                          <button
                            type="button"
                    ...
                    <div className="mt-6 pt-6 border-t border-[var(--border-color)] space-y-4">
                      ...
                      {(!selectedRegionId || currentRegions.length === 0) && (
                        <div className="mt-6 bg-[var(--bg-main)] border-2 border-blue-500/40 rounded-xl p-5 space-y-4 shadow-sm">
                          ...
                          {/* ٣ تێکست بۆکس بۆ ڕاستەوخۆ سەر ئاستی لق */}
                          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 bg-[var(--bg-card)] p-4 rounded-xl border border-[var(--border-color)]">
                            <div>
                              <label className="block text-xs font-bold text-emerald-600 mb-1">دەنگی دروست</label>
                              <input
                  ...
                  <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-6 shadow-sm space-y-6">
                    ...
                    <form onSubmit={handleCreateAccount} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      ...
                      {(newAccRole === 'branch_admin' || newAccRole === 'viewer') && (
                        <div>
                          <label className="block text-sm font-medium mb-1 text-[var(--text-secondary)]">{t.branchTabTitle}</label>
                          <select
                          ...
                          {users.map(u => {
                            ...
                            return (
                              <tr key={u.id} className="hover:bg-[var(--bg-hover)]">
                                ...
                                <td className="py-3 px-4 font-mono dir-ltr text-start">{u.username}</td>
                                <td className="py-3 px-4">
                                  <span className={`px-2 py-1 rounded text-xs font-bold ${
                                    ...
                                    {u.role === 'super_admin' ? t.superAdmin : u.role === 'branch_admin' ? t.branchAdminRole : t.viewerRole}
                                  </span>
                                </td>
                                <td className="py-3 px-4">{bObj ? bObj.name : t.generalOrNone}</td>
                                <td className="py-3 px-4">
                                  <input 
                                    type="text" 
                                    defaultValue={u.password}
                                    onBlur={(e) => handleAdminUpdateUserPassword(u.id, e.target.value)}
                                    className="bg-[var(--bg-main)] border border-[var(--border-color)] rounded px-2 py-1 text-xs font-mono w-28"
                                  />
                                </td>
                                <td className="py-3 px-4">
                                  {u.id !== 1 && (
              ...
              {currentUser.role === 'branch_admin' && (
                <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-6 shadow-sm space-y-4 max-w-lg">
                  ...
                  <form onSubmit={handleUpdateMyPassword} className="space-y-4">
                    ...
                    <button
                      type="submit"
                      className="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-6 py-2.5 rounded-lg text-sm shadow transition w-full"
                    >
            ...
            <div className="space-y-6">
              <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-6 shadow-sm space-y-6">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-[var(--border-color)] pb-4">
                  ...
                  <div className="flex items-center gap-3">
                    <button
                      ...
                      className="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-4 py-2 rounded-lg text-sm flex items-center gap-2 shadow"
                    >
                    ...
                    <button
                      ...
                      className="bg-emerald-600 hover:bg-emerald-700 text-white font-semibold px-4 py-2 rounded-lg text-sm flex items-center gap-2 shadow"
                    >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div>
                    <label className="block text-sm font-medium mb-1 text-[var(--text-secondary)]">{t.roundsTab}</label>
                    <select
                  ...
                  <div className="relative">
                    ...
                    <div 
                      onClick={() => {
                        if (repSelectedBranchIds.length > 0) {
                          setIsRepRegionOpen(!isRepRegionOpen);
                          setIsRepBranchOpen(false);
                        }
                      }}
                      className={`w-full bg-[var(--bg-main)] border border-[var(--border-color)] rounded-lg px-3 py-2 text-sm flex items-center justify-between select-none ${repSelectedBranchIds.length === 0 ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
                    >
                      <span className="truncate">
                    ...
                    {isRepRegionOpen && repSelectedBranchIds.length > 0 && (
                      <div className="absolute z-20 mt-1 w-full bg-[var(--bg-card)] border border-[var(--border-color)] rounded-lg shadow-xl p-3 space-y-2 max-h-60 overflow-y-auto">
                        <div className="flex items-center justify-between border-b border-[var(--border-color)] pb-2 mb-2">
                          <button
                            type="button"
              ...
              {/* ئەم بەشە داونلۆد دەبێت بۆ PDF یان چاپ دەکرێت */}
              <div ref={reportRef} className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-8 shadow-sm space-y-6 print:p-0 print:border-none">
                <div className="text-center border-b border-[var(--border-color)] pb-6 space-y-2">
                  <h1 className="text-2xl font-black text-blue-600">ڕاپۆرتی فەرمی شیکاری ئەنجامەکانی هەڵبژاردن</h1>
                  <p className="text-sm font-semibold">خولی هەڵبژاردن: {repSelectedRoundObj?.name || 'دیاری نەکراوە'}</p>
                  <p className="text-xs text-[var(--text-secondary)]">
                    بەڕێوەچوون: {repSelectedRoundObj?.date} | جۆر: {repSelectedRoundObj?.type} | ڕێکەوتی دەرچوواندنی ڕاپۆرت: {new Date().toLocaleDateString('fa-IR')}
                  </p>
                </div>

                <div className="space-y-4">
                  <h3 className="font-bold text-lg border-s-4 border-blue-600 ps-3">ئەنجامە گشتییەکانی دەنگی لایەنەکان</h3>
                  
                  <div className="overflow-x-auto">
                    <table className="w-full text-start border-collapse border border-[var(--border-color)]">
                      <thead>
                        <tr className="bg-[var(--bg-main)] border-b border-[var(--border-color)] text-sm">
                          ...
                          <th className="py-3 px-4 border-e border-[var(--border-color)] text-start">{t.votesCountLabel}</th>
                          <th className="py-3 px-4 text-start">{t.percentageLabel}</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-[var(--border-color)] text-sm">
                        {reportData.map((p, index) => (
                          <tr key={p.partyId} className="hover:bg-[var(--bg-hover)]">
                            <td className="py-3 px-4 border-e border-[var(--border-color)] font-bold text-[var(--text-secondary)]">{index + 1}</td>
                            <td className="py-3 px-4 border-e border-[var(--border-color)] font-semibold flex items-center gap-2">
                              <span className="w-3.5 h-3.5 rounded-full" style={{ backgroundColor: p.hexColor }}></span>
                              <span>{p.partyName}</span>
                            </td>
                            <td className="py-3 px-4 border-e border-[var(--border-color)] font-bold">{p.votes.toLocaleString()}</td>
                            <td className="py-3 px-4 font-extrabold text-blue-600">{p.percentage}%</td>
                          </tr>
                        ))}
                      </tbody>
                      <tfoot>
                        <tr className="bg-[var(--bg-main)] font-bold text-sm border-t-2 border-[var(--border-color)]">
                          <td colSpan={2} className="py-3 px-4 border-e border-[var(--border-color)] text-end">{t.totalVotesLabel}</td>
                          <td className="py-3 px-4 border-e border-[var(--border-color)] text-blue-600 font-extrabold">{totalReportVotes.toLocaleString()}</td>
                          <td className="py-3 px-4 text-blue-600 font-extrabold">100%</td>
                        </tr>
                      </tfoot>
                    </table>
                  </div>
                ...
                <div className="space-y-4 pt-4 border-t border-[var(--border-color)]">
                  <h3 className="font-bold text-lg border-s-4 border-blue-600 ps-3">هێڵکاری بەرراوردی ڕێژەی سەدی لایەنەکان</h3>
                  <div className="space-y-3 pt-2">
                    {reportData.map(item => (
                      <div key={item.partyName} className="space-y-1">
                        <div className="flex justify-between text-xs font-bold">
                          <span>{item.partyName}</span>
                          <span>{item.percentage}% ({item.votes.toLocaleString()} دەنگ)</span>
                        </div>
                        <div className="w-full bg-[var(--bg-main)] h-3.5 rounded-full overflow-hidden border border-[var(--border-color)]">
                          <div
                            className="h-full transition-all duration-500"
                            style={{ width: `${item.percentage}%`, backgroundColor: item.hexColor }}
                          ></div>
                ...
                <div className="pt-12 flex justify-between items-center text-xs text-[var(--text-secondary)] border-t border-[var(--border-color)]">
                  <div>
                    <p className="font-bold">مۆر و ئیمزای بەڕێوەبەرایەتی:</p>
                    <div className="h-12"></div>
                  </div>
                  <div className="text-end">
                    <p>سیستەمی شیکاری ئەنجامەکانی هەڵبژاردن</p>
                    <p>پەڕەی (1) لە (1)</p>
                  </div>
                </div>
              </div>
