      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="flex-1 p-6 overflow-y-auto space-y-6">
                      ...
                      {(!selectedRegionId || currentRegions.length === 0) && (
                        <div className="mt-6 bg-[var(--bg-main)] border-2 border-blue-500/30 rounded-xl p-5 space-y-4">
                          <div className="flex items-center justify-between">
                            ...
                            <span className="text-xs bg-blue-600 text-white px-2.5 py-1 rounded-md font-semibold">
                              {branches.find(b => b.id === selectedBranchId)?.name}
                            </span>
                          </div>

                          {/* ٣ تێکست بۆکس ڕاستەوخۆ لەسەر ئاستی لق */}
                          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 bg-[var(--bg-card)] p-4 rounded-xl border border-[var(--border-color)]">
                            <div>
                              <label className="block text-xs font-bold text-emerald-600 mb-1">دەنگی دروست (کۆی لق)</label>
                              <input
                                type="number"
                            ...
                            <div>
                              <label className="block text-xs font-bold text-rose-600 mb-1">دەنگی سووتاو (کۆی لق)</label>
                              <input
                                type="number"
                            ...
                            <div>
                              <label className="block text-xs font-bold text-blue-600 mb-1">کۆی گشتی دەنگدەر (لق)</label>
                              <input
                                type="number"
          ...
          {activeMainTab === 'controlPanel' && currentUser.role === 'super_admin' && (
            <div className="space-y-6">
              <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-6 shadow-sm space-y-4">
                ...
                <form onSubmit={handleCreateAccount} className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-[var(--bg-main)] border border-[var(--border-color)] rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-amber-500"
                  ...
                  {(newAccRole === 'branch_admin' || newAccRole === 'viewer') && (
                    <div className="md:col-span-2">
                      <label className="block text-sm font-medium mb-1 text-[var(--text-secondary)]">لقی پەیوەندیدار</label>
                      <select
                  ...
                  <div className="md:col-span-2 flex justify-end">
                    <button
                      type="submit"
                      className="bg-amber-600 hover:bg-amber-700 text-white font-semibold px-6 py-2.5 rounded-lg text-sm shadow transition-all"
                    >
                      {t.createAccountBtn}
                    </button>
                  </div>
              ...
              <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-6 shadow-sm space-y-4">
                <h2 className="text-xl font-bold">{t.accountsListTitle}</h2>
                <div className="overflow-x-auto">
                  <table className="w-full text-start border-collapse">
                    <thead>
                      <tr className="border-b border-[var(--border-color)] text-[var(--text-secondary)] text-sm">
                        ...
                        <tr key={u.id} className="hover:bg-[var(--bg-hover)]">
                          ...
                          <td className="py-3 px-4">
                            <input 
                              ...
                              className="bg-[var(--bg-main)] border border-[var(--border-color)] rounded px-2 py-1 text-xs w-28 focus:ring-1 focus:ring-amber-500 font-mono"
                            />
                          </td>
                          <td className="py-3 px-4">
                            {u.role === 'super_admin' ? (
                              <span className="bg-amber-100 text-amber-800 dark:bg-amber-950 dark:text-amber-300 px-2 py-1 rounded text-xs font-bold">{t.superAdmin}</span>
                            ) : u.role === 'branch_admin' ? (
                              <span className="bg-blue-100 text-blue-800 dark:bg-blue-950 dark:text-blue-300 px-2 py-1 rounded text-xs font-bold">{t.branchAdmin}</span>
                            ) : (
                              <span className="bg-gray-100 text-gray-800 dark:bg-gray-800 dark:text-gray-300 px-2 py-1 rounded text-xs font-bold">{t.viewer}</span>
                            )}
                          </td>
                          <td className="py-3 px-4">
                            {branches.find(b => b.id === u.branchId)?.name || t.generalOrNone}
                          </td>
                          <td className="py-3 px-4">
                            {u.id !== 1 && (
                              <button
          ...
          {activeMainTab === 'controlPanel' && currentUser.role === 'branch_admin' && (
            <div className="space-y-6">
              <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-6 shadow-sm space-y-4 max-w-lg">
                ...
                <form onSubmit={handleUpdateMyPassword} className="space-y-4 pt-2">
                  <div>
                  ...
                  <button
                    type="submit"
                    className="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2.5 rounded-lg text-sm shadow transition-all"
                  >
          ...
          {/* تابی ڕاپۆرتەکان تەنها بۆ super_admin و branch_admin */}
            ...
            <div className="space-y-6">
              <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-6 shadow-sm space-y-6 print:hidden">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-[var(--border-color)] pb-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-lg text-sm font-semibold shadow transition"
                    >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div>
                    <label className="block text-sm font-medium mb-1 text-[var(--text-secondary)]">خولی هەڵبژاردن</label>
                    <select
                      ...
                      className="w-full bg-[var(--bg-main)] border border-[var(--border-color)] rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-emerald-500"
                    >
                      {rounds.map(r => (
              ...
              <div ref={reportRef} className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-8 shadow-sm space-y-6 print:border-none print:shadow-none print:p-0">
                <div className="text-center border-b border-[var(--border-color)] pb-6 space-y-2">
                  ...
                  <h2 className="text-lg font-semibold text-blue-600">
                    ڕاپۆرتی ئەنجامەکانی: {repSelectedRoundObj?.name || 'دیاری نەکراوە'}
                  </h2>
                  <p className="text-xs text-[var(--text-secondary)]">
                    بەرواری دەرچوونی ڕاپۆرت: {new Date().toLocaleDateString('ar-EG')}
                  </p>
                </div>

                <div className="flex justify-between items-center bg-[var(--bg-main)] p-4 rounded-xl border border-[var(--border-color)]">
                  <div>
                    <span className="text-xs text-[var(--text-secondary)] block">کۆی گشتی دەنگە دروستەکان لەم ڕاپۆرتەدا</span>
                    <span className="text-2xl font-extrabold text-emerald-600">{totalReportVotes.toLocaleString()}</span>
                  </div>
                  <div className="text-end">
                    <span className="text-xs text-[var(--text-secondary)] block">جۆری هەڵبژاردن</span>
                    <span className="text-sm font-bold">{repSelectedRoundObj?.type || 'دیاری نەکراوە'}</span>
                  </div>
                </div>

                <div className="space-y-4">
                  <h3 className="text-base font-bold border-b border-[var(--border-color)] pb-2">دەنگی لایەنەکان</h3>
                  <div className="overflow-x-auto">
                    <table className="w-full text-start border-collapse">
                      <thead>
                        <tr className="border-b border-[var(--border-color)] text-[var(--text-secondary)] text-sm bg-[var(--bg-main)]">
                          <th className="py-3 px-4">#</th>
                          <th className="py-3 px-4">{t.partyNameHeader}</th>
                          <th className="py-3 px-4">{t.votesCountLabel}</th>
                      ...
                      <tbody className="divide-y divide-[var(--border-color)] text-sm">
                        {reportData.map((item, idx) => (
                          <tr key={item.partyId} className="hover:bg-[var(--bg-hover)]">
                            <td className="py-3 px-4 font-bold text-[var(--text-secondary)]">{idx + 1}</td>
                            <td className="py-3 px-4 font-semibold flex items-center gap-2">
                              <span className="w-3 h-3 rounded-full shrink-0" style={{ backgroundColor: item.hexColor }}></span>
                              <span>{item.partyName}</span>
                            </td>
                            <td className="py-3 px-4 font-bold">{item.votes.toLocaleString()}</td>
                            <td className="py-3 px-4 font-bold text-blue-600">{item.percentage}%</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
