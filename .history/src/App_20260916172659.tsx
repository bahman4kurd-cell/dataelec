        ...
        <main className="flex-1 p-6 overflow-y-auto space-y-6">
                      ...
                      {(!selectedRegionId || currentRegions.length === 0) && (
                        <div className="mt-6 bg-[var(--bg-main)] border-2 border-blue-500/30 rounded-xl p-5 space-y-4">
                          <div className="flex items-center justify-between">
                            ...
                            <span className="text-xs font-bold bg-blue-100 text-blue-700 dark:bg-blue-950 dark:text-blue-300 px-2.5 py-1 rounded-full">
                              {t.selectedBranchBadge}
                            </span>
                          </div>

                          {/* ٣ تێکست بۆکس بۆ لقەکە کاتێک هیچ ناوچەیەکی نییە */}
                          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 bg-[var(--bg-card)] p-4 rounded-xl border border-[var(--border-color)]">
                            <div>
                              <label className="block text-xs font-bold text-emerald-600 mb-1">دەنگی دروست</label>
                              <input
                    ...
                    <form onSubmit={handleCreateAccount} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      <div>
                        ...
                        <input
                          ...
                          placeholder="ناوی ناوەند/لق یان بەکارهێنەر"
                          className="w-full bg-[var(--bg-main)] border border-[var(--border-color)] rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500"
                          required
                        />
                      </div>
                      ...
                      <div className="md:col-span-2 lg:col-span-3 flex justify-end">
                        <button
                          type="submit"
                          className="bg-emerald-600 hover:bg-emerald-700 text-white font-semibold px-6 py-2.5 rounded-lg text-sm shadow transition-all"
                        >
                          {t.createAccountBtn}
                        </button>
                      </div>
                  ...
                  <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-6 shadow-sm space-y-4">
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full text-start border-collapse">
                        ...
                        <tbody className="divide-y divide-[var(--border-color)] text-sm">
                          {users.map((u) => {
                            const bObj = branches.find(b => b.id === u.branchId);
                            return (
                              <tr key={u.id} className="hover:bg-[var(--bg-hover)]">
                                ...
                                <td className="py-3 px-4">
                                  <span className={`px-2.5 py-1 rounded-full text-xs font-bold ${
                                    ...
                                    u.role === 'branch_admin' ? 'bg-blue-100 text-blue-700 dark:bg-blue-950 dark:text-blue-300' :
                                ...
                                <td className="py-3 px-4 font-mono">
                                  <input 
                                    ...
                                    className="bg-[var(--bg-main)] border border-[var(--border-color)] rounded px-2 py-1 text-xs w-28 focus:ring-1 focus:ring-blue-500"
                                  />
                                </td>
                                <td className="py-3 px-4 text-xs font-medium">
                                  {bObj ? bObj.name : t.generalOrNone}
                                </td>
                                <td className="py-3 px-4">
                                  {u.id !== 1 && (
                                    <button
              ...
              {currentUser.role === 'branch_admin' && (
                <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-6 shadow-sm space-y-4 max-w-xl">
                  ...
                  <form onSubmit={handleUpdateMyPassword} className="space-y-4">
                    ...
                    <button
                      type="submit"
                      className="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-6 py-2.5 rounded-lg text-sm shadow transition-all"
                    >
          ...
          {activeMainTab === 'reports' && currentUser.role !== 'viewer' && (
            <div className="space-y-6">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-[var(--border-color)] pb-4">
                  ...
                  <div className="flex items-center gap-3">
                    <button
                      ...
                      className="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-4 py-2 rounded-xl text-sm shadow flex items-center gap-2 transition"
                    >
                      <span>{t.printReportBtn}</span>
                    </button>
                    <button
                      onClick={handleDownloadPdf}
                      className="bg-emerald-600 hover:bg-emerald-700 text-white font-semibold px-4 py-2 rounded-xl text-sm shadow flex items-center gap-2 transition"
                    >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div>
                    ...
                    <select
                      ...
                      className="w-full bg-[var(--bg-main)] border border-[var(--border-color)] rounded-lg px-3 py-2 text-sm font-semibold focus:ring-2 focus:ring-blue-500"
                    >
                      {rounds.map(r => (
              ...
              {/* بەشی راپۆرتەکە کە دەکرێت بە PDF یان چاپدەکرێت */}
              <div ref={reportRef} className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-8 shadow-sm space-y-6 print:p-0 print:border-none print:shadow-none">
                <div className="text-center space-y-2 border-b border-[var(--border-color)] pb-6">
                  <h1 className="text-2xl font-extrabold text-[var(--text-primary)]">{t.systemTitle}</h1>
                  <h3 className="text-lg font-bold text-blue-600">{repSelectedRoundObj?.name || 'ڕاپۆرتی گشتی'}</h3>
                  <p className="text-xs text-[var(--text-secondary)]">
                    بەرواری دەرهێنانی ڕاپۆرت: {new Date().toLocaleDateString('en-GB')}
                  </p>
                </div>

                <div className="space-y-4">
                  <div className="flex justify-between items-center">
                    <h3 className="text-base font-bold">{t.reportsTitle}</h3>
                    <span className="text-sm font-bold bg-blue-100 text-blue-800 dark:bg-blue-950 dark:text-blue-300 px-3 py-1 rounded-full">
                      {t.totalVotesLabel} {totalReportVotes.toLocaleString()}
                    </span>
                  </div>
                  ...
                  <div className="overflow-x-auto border border-[var(--border-color)] rounded-xl">
                    <table className="w-full text-start border-collapse">
                      <thead>
                        <tr className="bg-[var(--bg-main)] border-b border-[var(--border-color)] text-[var(--text-secondary)] text-sm">
                      ...
                      <tbody className="divide-y divide-[var(--border-color)] text-sm">
                        {reportData.map((item, idx) => (
                          <tr key={item.partyId} className="hover:bg-[var(--bg-hover)]">
                            ...
                            <td className="py-3 px-4 font-extrabold text-blue-600">{item.percentage}%</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                ...
                <div className="pt-6 border-t border-[var(--border-color)] space-y-4">
                  <h4 className="text-base font-bold text-[var(--text-primary)]">دیاریدانی دەنگی لایەنەکان بەشێوازی چارتی دۆنات</h4>
                  <div className="flex flex-col md:flex-row items-center justify-around gap-6 bg-[var(--bg-main)] p-6 rounded-2xl border border-[var(--border-color)]">
                    <div className="w-64 h-64 relative flex items-center justify-center">
                      <svg viewBox="0 0 100 100" className="w-full h-full drop-shadow">
                        {createSvgSlices(reportData, true)}
                      </svg>
                      <div className="absolute inset-0 flex flex-col items-center justify-center text-center pointer-events-none">
                        <span className="text-xs font-semibold text-[var(--text-secondary)]">کۆی دەنگ</span>
                        <span className="text-sm font-extrabold text-[var(--text-primary)]">{totalReportVotes.toLocaleString()}</span>
                      </div>
                    </div>

                    <div className="grid grid-cols-2 gap-2 w-full md:w-auto">
                      {reportData.map(item => (
                        <div key={item.partyId} className="flex items-center gap-2 text-xs font-semibold bg-[var(--bg-card)] p-2 rounded-lg border border-[var(--border-color)]">
                          ...
                          <span className="text-blue-600 font-bold ms-auto">{item.percentage}%</span>
                        </div>
                      ))}
                    </div>
                  </div>
