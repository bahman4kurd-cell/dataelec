...
// ئینپۆرتکردنی کتێبخانەی html2pdf.js بۆ بەستنەوەی فەنکشنی داونلۆدی PDF
...
export function App() {
  ...
  // ئەگەر یوزەرەکە viewer بوو و تابی ڕاپۆرت هەڵبژێردرابوو، بیگوازەوە سەر داشبۆرد بە شێوازی سەلامەت
  useEffect(() => {
  ...
  // زانیاری دەنگی دروست، دەنگی سووتاو، کۆی دەنگدەران بۆ لق و ناوچەکان
  ...
  // گۆڕینی فلتەری لق و ناوچەکان بۆ Multi-select (Arrays)
  ...
  // فلتەرەکانی تایبەت بە تابی ڕاپۆرتەکان
  ...
  // بەستنەوەی ڕێفرنس بۆ ئەو بەشەی کە دەکرێت بە PDF بەپێی کۆدی داواکراو
  ...
  // دیاریکردنی لق بۆ ئەدمینی لق و چاودێر
  useEffect(() => {
  ...
  // نوێکردنەوەی هەڵبژاردنی لقەکان کاتێک خول دەگۆڕێت
  useEffect(() => {
  ...
  // نوێکردنەوەی لقەکانی ڕاپۆرت کاتێک خولی ڕاپۆرت دەگۆڕێت
  useEffect(() => {
  ...
  const handleBranchMetaChange = (branchId: number, key: keyof BranchVoteMetaData, val: number) => {
    ...
    // دووبارە پۆلێنکردنەوە و ئاڵوگۆڕی ڕێژەی سەدی لایەنەکان بەپێی "دەنگی دروست"
    const currentList = branchVotes[branchId] || defaultParties.map(p => ({ ...p, votes: 0, percentage: 0 }));
  ...
  const getFilteredDashboardVotes = () => {
    ...
    // دیاریکردنی هەموو ئەو لقانەی سەر بەم خولەن ئەگەر هیچ لقێک دەستنیشان نەکرابێت
    ...
    <div className="min-h-screen flex flex-col bg-[var(--bg-main)] text-[var(--text-primary)] transition-colors duration-200" dir={getDir()}>
      ...
      <div className="flex flex-1 overflow-hidden">
        <aside className="w-64 bg-[var(--bg-card)] border-e border-[var(--border-color)] p-4 flex flex-col gap-2 shadow-sm print:hidden">
          ...
          {/* نیشدانی تابی ڕاپۆرتەکان تەنها ئەگەر یوزەرەکە viewer نەبێت */}
        ...
        <main className="flex-1 p-6 overflow-y-auto space-y-6">
                      ...
                      rounds.map(r => (
                        <button
                          ...
                          onClick={() => {
                            ...
                            if (currentUser.role !== 'branch_admin' && currentUser.role !== 'viewer') {
                              const rB = branches.filter(b => b.roundId === r.id).map(b => b.id);
                              setDashSelectedBranchIds(rB);
                            }
                            setDashSelectedRegionIds([]);
                          }}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2">
                  <div className="relative">
                    ...
                    <div 
                      ...
                      className="w-full bg-[var(--bg-main)] border border-[var(--border-color)] rounded-lg px-3 py-2 text-sm flex items-center justify-between cursor-pointer select-none"
                    >
                      <span className="truncate">
                        {dashSelectedBranchIds.length === 0 
                          ? t.allBranchesOption
                          : dashSelectedBranchIds.length === dashFilteredBranches.length
                          ? t.allBranchesOption
                          : `${dashSelectedBranchIds.length} لق هەڵبژێردراوە`}
                      </span>
                      <span>▼</span>
                    </div>

                    {isBranchDropdownOpen && (
                      <div className="absolute z-20 mt-1 w-full bg-[var(--bg-card)] border border-[var(--border-color)] rounded-lg shadow-xl p-3 space-y-2 max-h-60 overflow-y-auto">
                        <div className="flex items-center justify-between border-b border-[var(--border-color)] pb-2 mb-2">
                          ...
                          <button
                            type="button"
                            onClick={() => { setDashSelectedBranchIds([]); setDashSelectedRegionIds([]); }}
                            className="text-xs text-red-500 font-bold hover:underline"
                          >
                            هیچیان هەڵمەبژێرە
                          </button>
                        </div>
                        {dashFilteredBranches.map(branch => {
                          const isChecked = dashSelectedBranchIds.includes(branch.id);
                          return (
                            <div 
                              key={branch.id}
                              onClick={() => {
                                if (isChecked) {
                                  const updated = dashSelectedBranchIds.filter(id => id !== branch.id);
                                  setDashSelectedBranchIds(updated);
                                  const branchRegIds = regions.filter(reg => reg.branchId === branch.id).map(r => r.id);
                                  setDashSelectedRegionIds(dashSelectedRegionIds.filter(id => !branchRegIds.includes(id)));
                                } else {
                                  setDashSelectedBranchIds([...dashSelectedBranchIds, branch.id]);
                                }
                              }}
                              className="flex items-center gap-2 text-xs font-semibold cursor-pointer hover:bg-[var(--bg-hover)] p-1.5 rounded"
                            >
                              {isChecked ? (
                  ...
                  <div className="relative">
                    ...
                    <div 
                      onClick={() => {
                        if (dashFilteredBranches.length > 0) {
                          setIsRegionDropdownOpen(!isRegionDropdownOpen);
                          setIsBranchDropdownOpen(false);
                          setIsPartyDropdownOpen(false);
                        }
                      }}
                      className={`w-full bg-[var(--bg-main)] border border-[var(--border-color)] rounded-lg px-3 py-2 text-sm flex items-center justify-between select-none ${dashFilteredBranches.length === 0 ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
                    >
                      <span className="truncate">
                        {dashSelectedRegionIds.length === 0 
                          ? t.allAreasOption
                          : dashSelectedRegionIds.length === dashFilteredRegions.length
                          ? 'هەموو ناوچەکان'
                          : `${dashSelectedRegionIds.length} ناوچە هەڵبژێردراوە`}
                      </span>
                      <span>▼</span>
                    </div>
                    ...
                    {isRegionDropdownOpen && (
                      <div className="absolute z-20 mt-1 w-full bg-[var(--bg-card)] border border-[var(--border-color)] rounded-lg shadow-xl p-3 space-y-2 max-h-60 overflow-y-auto">
                        ...
                        {dashFilteredRegions.length === 0 ? (
                          <p className="text-xs text-gray-500">هیچ ناوچەیەک بۆ لقە هەڵبژاردراوەکان نییە</p>
                        ) : (
                          dashFilteredRegions.map(reg => {
                            const isChecked = dashSelectedRegionIds.includes(reg.id);
                            return (
                              <div 
                                key={reg.id}
                                onClick={() => {
                  ...
                  <div className="relative">
                    <label className="block text-sm font-medium mb-1 text-[var(--text-secondary)]">{t.filterPartyLabel}</label>
                    <div 
                      onClick={() => {
                    ...
                    >
                      <span className="truncate">
                        {selectedPartyIds.length === defaultParties.length 
                          ? t.selectAllParties 
                          : `${selectedPartyIds.length} لایەن هەڵبژێردراوە`}
                      </span>
                      <span>▼</span>
                    </div>
                    ...
                    {isPartyDropdownOpen && (
                      <div className="absolute z-20 mt-1 w-full bg-[var(--bg-card)] border border-[var(--border-color)] rounded-lg shadow-xl p-3 space-y-2 max-h-60 overflow-y-auto">
                        ...
                        {defaultParties.map(party => {
                          const isChecked = selectedPartyIds.includes(party.partyId);
                          return (
                            <div 
                              key={party.partyId}
                              onClick={() => {
                  ...
                  <form onSubmit={handleSaveRound} className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div>
                      ...
                      <select
                        ...
                        className="w-full bg-[var(--bg-main)] border border-[var(--border-color)] rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500"
                      >
                        <option value="پەرلەمانی">پەرلەمان</option>
                        <option value="ئەنجومەنی پارێزگاکان">ئەنجومەنی پارێزگاکان</option>
                        <option value="سەرۆکایەتی">سەرۆکایەتیەکان(کۆمار،هەرێم،شارەوان)</option>
                      </select>
                    </div>
                    <div>
                ...
                <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-6 shadow-sm space-y-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {currentBranches.length === 0 ? (
                      <p className="text-sm text-[var(--text-secondary)]">هیچ لقێک بۆ ئەم خولە بەردەست نییە یان بۆت دیاری نەکراوە.</p>
                    ) : (
                    ...
                    <div className="mt-6 pt-6 border-t border-[var(--border-color)] space-y-4">
                      ...
                      {selectedRegionId && (
                        <div className="mt-4 bg-[var(--bg-main)] border border-[var(--border-color)] rounded-xl p-5 space-y-4">
                          ...
                          {/* ٣ تێکست بۆکس بۆ ناوچەی دیاریکراو */}
                          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 bg-[var(--bg-card)] p-4 rounded-xl border border-[var(--border-color)]">
                            <div>
                              <label className="block text-xs font-bold text-emerald-600 mb-1">دەنگی دروست</label>
                              <input
                                type="number"
                                value={regionMeta[selectedRegionId]?.validVotes || 0}
                                disabled={currentUser.role === 'viewer'}
                                onChange={(e) => handleRegionMetaChange(selectedRegionId, 'validVotes', Number(e.target.value) || 0)}
                                className="w-full bg-[var(--bg-main)] border border-[var(--border-color)] rounded-md px-3 py-1.5 text-sm font-semibold focus:ring-2 focus:ring-emerald-500 disabled:opacity-60"
                              />
                            </div>
                            <div>
                              <label className="block text-xs font-bold text-rose-600 mb-1">دەنگی سووتاو</label>
                              <input
                                type="number"
                                value={regionMeta[selectedRegionId]?.invalidVotes || 0}
                                disabled={currentUser.role === 'viewer'}
                                onChange={(e) => handleRegionMetaChange(selectedRegionId, 'invalidVotes', Number(e.target.value) || 0)}
                                className="w-full bg-[var(--bg-main)] border border-[var(--border-color)] rounded-md px-3 py-1.5 text-sm font-semibold focus:ring-2 focus:ring-rose-500 disabled:opacity-60"
                              />
                            </div>
                            <div>
                              <label className="block text-xs font-bold text-blue-600 mb-1">کۆی گشتی دەنگدەر</label>
                              <input
                                type="number"
                                value={regionMeta[selectedRegionId]?.totalVoters || 0}
                                disabled={currentUser.role === 'viewer'}
                                onChange={(e) => handleRegionMetaChange(selectedRegionId, 'totalVoters', Number(e.target.value) || 0)}
                                className="w-full bg-[var(--bg-main)] border border-[var(--border-color)] rounded-md px-3 py-1.5 text-sm font-semibold focus:ring-2 focus:ring-blue-500 disabled:opacity-60"
                              />
                            </div>
                          </div>
                      ...
                      {(!selectedRegionId || currentRegions.length === 0) && (
                        <div className="mt-6 bg-[var(--bg-main)] border-2 border-blue-500/30 rounded-xl p-5 space-y-4">
                          <div className="flex items-center justify-between">
                            <h4 className="font-bold text-base text-[var(--text-primary)]">
                              {t.directBranchEntry} ({branches.find(b => b.id === selectedBranchId)?.name})
                            </h4>
                          ...
                          {/* ٣ تێکست بۆکس بۆ سەر ئاستی لق */}
                          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 bg-[var(--bg-card)] p-4 rounded-xl border border-[var(--border-color)]">
                            <div>
                              <label className="block text-xs font-bold text-emerald-600 mb-1">دەنگی دروست</label>
                              <input
                                type="number"
                                value={branchMeta[selectedBranchId]?.validVotes || 0}
                                disabled={currentUser.role === 'viewer'}
                                onChange={(e) => handleBranchMetaChange(selectedBranchId, 'validVotes', Number(e.target.value) || 0)}
                                className="w-full bg-[var(--bg-main)] border border-[var(--border-color)] rounded-md px-3 py-1.5 text-sm font-semibold focus:ring-2 focus:ring-emerald-500 disabled:opacity-60"
                              />
                            </div>
                            <div>
                              ...
                              <input
                                type="number"
                                value={branchMeta[selectedBranchId]?.invalidVotes || 0}
                                disabled={currentUser.role === 'viewer'}
                                onChange={(e) => handleBranchMetaChange(selectedBranchId, 'invalidVotes', Number(e.target.value) || 0)}
                                className="w-full bg-[var(--bg-main)] border border-[var(--border-color)] rounded-md px-3 py-1.5 text-sm font-semibold focus:ring-2 focus:ring-rose-500 disabled:opacity-60"
                              />
                            </div>
                            <div>
                              ...
                              <input
                                type="number"
                                value={branchMeta[selectedBranchId]?.totalVoters || 0}
                                disabled={currentUser.role === 'viewer'}
                                onChange={(e) => handleBranchMetaChange(selectedBranchId, 'totalVoters', Number(e.target.value) || 0)}
                                className="w-full bg-[var(--bg-main)] border border-[var(--border-color)] rounded-md px-3 py-1.5 text-sm font-semibold focus:ring-2 focus:ring-blue-500 disabled:opacity-60"
                              />
                            </div>
                          </div>
          ...
          {activeMainTab === 'controlPanel' && currentUser.role === 'super_admin' && (
            <div className="space-y-6">
              <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-6 shadow-sm space-y-6">
                <div>
                  <h2 className="text-xl font-bold flex items-center gap-2">
                    <Users className="w-6 h-6 text-amber-500" />
                    <span>{t.controlPanelTitle}</span>
                  </h2>
                  <p className="text-sm text-[var(--text-secondary)] mt-1">{t.controlPanelSubtitle}</p>
                </div>

                {accSuccessMsg && (
                  <div className="p-3 bg-emerald-950/50 border border-emerald-800 text-emerald-300 rounded-xl text-sm font-semibold">
                    {accSuccessMsg}
                  </div>
                )}

                {loginError && (
                  <div className="p-3 bg-red-950/50 border border-red-800 text-red-300 rounded-xl text-sm font-semibold">
                    {loginError}
                  </div>
                )}
                ...
                <form onSubmit={handleCreateAccount} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-[var(--text-secondary)] mb-1">{t.fullNameLabel}</label>
                    <input
                      type="text"
                      value={newAccName}
                      onChange={(e) => setNewAccName(e.target.value)}
                      placeholder="ناوی ستاف / لق"
                      className="w-full bg-[var(--bg-main)] border border-[var(--border-color)] rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-amber-500"
                      required
                    />
                  </div>
                  ...
                  <div>
                    <label className="block text-sm font-medium text-[var(--text-secondary)] mb-1">{t.roleLabel}</label>
                    <select
                      value={newAccRole}
                      onChange={(e) => setNewAccRole(e.target.value as UserRole)}
                      className="w-full bg-[var(--bg-main)] border border-[var(--border-color)] rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-amber-500 font-semibold"
                    >
                  ...
                  {(newAccRole === 'branch_admin' || newAccRole === 'viewer') && (
                    <div>
                      <label className="block text-sm font-medium text-[var(--text-secondary)] mb-1">{t.filterBranchLabel}</label>
                      <select
                      ...
                      >
                        ...
                        {branches.map(b => (
                          <option key={b.id} value={b.id}>{b.name} ({rounds.find(r => r.id === b.roundId)?.name})</option>
                        ))}
                      </select>
                    </div>
                  )}
                  ...
                  <div className="md:col-span-2 lg:col-span-3 flex justify-end mt-2">
                    <button
                      type="submit"
                      className="bg-amber-600 hover:bg-amber-700 text-white font-bold px-6 py-2.5 rounded-xl text-sm shadow-md transition"
                    >
                      {t.createAccountBtn}
                    </button>
                  </div>
                          ...
                          <tr key={u.id} className="hover:bg-[var(--bg-hover)]">
                            ...
                            <td className="py-3 px-4">
                              <input
                                ...
                                className="bg-[var(--bg-main)] border border-[var(--border-color)] px-2 py-1 rounded text-xs font-mono font-bold w-28 focus:ring-2 focus:ring-blue-500"
                              />
                            </td>
                            <td className="py-3 px-4">
                              <span className={`px-2.5 py-1 rounded-full text-xs font-bold ${
                                u.role === 'super_admin' ? 'bg-red-100 text-red-800' : u.role === 'branch_admin' ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800'
                              }`}>
                                {u.role === 'super_admin' ? t.superAdmin : u.role === 'branch_admin' ? t.branchAdmin : t.viewer}
                              </span>
                            </td>
                            <td className="py-3 px-4">{linkedB ? linkedB.name : t.generalOrNone}</td>
                            <td className="py-3 px-4">
                              {u.id !== 1 && (
                                <button
          ...
          {activeMainTab === 'controlPanel' && currentUser.role === 'branch_admin' && (
            <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-6 shadow-sm space-y-6 max-w-xl">
              <div>
                <h2 className="text-xl font-bold flex items-center gap-2">
                  <Key className="w-6 h-6 text-blue-500" />
                  <span>{t.changeMyPasswordTitle}</span>
                </h2>
              ...
              <form onSubmit={handleUpdateMyPassword} className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-[var(--bg-main)] border border-[var(--border-color)] rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500"
                    required
                  />
                </div>
          ...
          {activeMainTab === 'reports' && currentUser.role !== 'viewer' && (
            <div className="space-y-6">
              <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-6 shadow-sm space-y-6 print:hidden">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-[var(--border-color)] pb-4">
                  <div>
                    <h2 className="text-xl font-bold flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-3">
                    <button
                      ...
                      className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-xl text-sm font-bold transition shadow"
                    >
                    ...
                    <button
                      ...
                      className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-xl text-sm font-bold transition shadow"
                    >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div>
                    <label className="block text-sm font-medium mb-1 text-[var(--text-secondary)]">{t.selectedRound}</label>
                    <select
                      value={repSelectedRoundId || ''}
                      onChange={(e) => {
                        const rId = Number(e.target.value) || null;
                        setRepSelectedRoundId(rId);
                        if (currentUser.role !== 'branch_admin' && currentUser.role !== 'viewer') {
                          const rB = branches.filter(b => b.roundId === rId).map(b => b.id);
                          setRepSelectedBranchIds(rB);
                        }
                        setRepSelectedRegionIds([]);
                      }}
                      className="w-full bg-[var(--bg-main)] border border-[var(--border-color)] rounded-lg px-3 py-2 text-sm font-bold focus:ring-2 focus:ring-emerald-500"
                    >
                      {rounds.map(r => (
                  ...
                  <div className="relative">
                    ...
                    <div 
                      onClick={() => {
                        if (currentUser.role !== 'branch_admin' && currentUser.role !== 'viewer') {
                          setIsRepBranchOpen(!isRepBranchOpen);
                          setIsRepRegionOpen(false);
                        }
                      }}
                      className="w-full bg-[var(--bg-main)] border border-[var(--border-color)] rounded-lg px-3 py-2 text-sm flex items-center justify-between cursor-pointer select-none"
                    >
                      <span className="truncate">
                        {repSelectedBranchIds.length === 0 
                          ? t.allBranchesOption
                          : repSelectedBranchIds.length === repFilteredBranches.length
                          ? t.allBranchesOption
                          : `${repSelectedBranchIds.length} لق هەڵبژێردراوە`}
                      </span>
                      <span>▼</span>
                    </div>
                    ...
                    {isRepBranchOpen && (
                      <div className="absolute z-20 mt-1 w-full bg-[var(--bg-card)] border border-[var(--border-color)] rounded-lg shadow-xl p-3 space-y-2 max-h-60 overflow-y-auto">
                        <div className="flex items-center justify-between border-b border-[var(--border-color)] pb-2 mb-2">
                          ...
                          <button
                            type="button"
                            onClick={() => { setRepSelectedBranchIds([]); setRepSelectedRegionIds([]); }}
                            className="text-xs text-red-500 font-bold hover:underline"
                          >
                            هیچیان هەڵمەبژێرە
                          </button>
                        </div>
                        {repFilteredBranches.map(branch => {
                          const isChecked = repSelectedBranchIds.includes(branch.id);
                          return (
                            <div 
                              key={branch.id}
                              onClick={() => {
                                if (isChecked) {
                                  const updated = repSelectedBranchIds.filter(id => id !== branch.id);
                                  setRepSelectedBranchIds(updated);
                                  const branchRegIds = regions.filter(reg => reg.branchId === branch.id).map(r => r.id);
                                  setRepSelectedRegionIds(repSelectedRegionIds.filter(id => !branchRegIds.includes(id)));
                                } else {
                                  setRepSelectedBranchIds([...repSelectedBranchIds, branch.id]);
                                }
                              }}
                              className="flex items-center gap-2 text-xs font-semibold cursor-pointer hover:bg-[var(--bg-hover)] p-1.5 rounded"
                            >
                  ...
                  <div className="relative">
                    ...
                    <div 
                      onClick={() => {
                        if (repFilteredBranches.length > 0) {
                          setIsRepRegionOpen(!isRepRegionOpen);
                          setIsRepBranchOpen(false);
                        }
                      }}
                      className={`w-full bg-[var(--bg-main)] border border-[var(--border-color)] rounded-lg px-3 py-2 text-sm flex items-center justify-between select-none ${repFilteredBranches.length === 0 ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
                    >
                      <span className="truncate">
                        {repSelectedRegionIds.length === 0 
                          ? t.allAreasOption
                          : repSelectedRegionIds.length === repFilteredRegions.length
                          ? 'هەموو ناوچەکان'
                          : `${repSelectedRegionIds.length} ناوچە هەڵبژێردراوە`}
                      </span>
                      <span>▼</span>
                    </div>
                    ...
                    {isRepRegionOpen && (
                      <div className="absolute z-20 mt-1 w-full bg-[var(--bg-card)] border border-[var(--border-color)] rounded-lg shadow-xl p-3 space-y-2 max-h-60 overflow-y-auto">
                        ...
                        {repFilteredRegions.length === 0 ? (
                          <p className="text-xs text-gray-500">هیچ ناوچەیەک بۆ لقە هەڵبژاردراوەکان نییە</p>
                        ) : (
                          repFilteredRegions.map(reg => {
                            const isChecked = repSelectedRegionIds.includes(reg.id);
                            return (
                              <div 
                                key={reg.id}
                                onClick={() => {
              ...
              {/* پەڕەی ڕاپۆرتی فەرمی کە هەم پرنت دەبێت و هەم داونلۆدی PDF دەکرێت */}
              <div ref={reportRef} className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-2xl p-8 shadow-md space-y-8 print:border-none print:shadow-none print:p-0">
                <div className="text-center border-b border-[var(--border-color)] pb-6 space-y-2">
                  <h1 className="text-2xl font-black">{t.systemTitle}</h1>
                  <h3 className="text-lg font-bold text-blue-600">{repSelectedRoundObj?.name || 'هیچ خولێک دیاری نەکراوە'}</h3>
                  <div className="flex justify-center items-center gap-6 text-xs text-[var(--text-secondary)] font-semibold pt-2">
                    <span>بەڕێوەچوون: {repSelectedRoundObj?.date || '---'}</span>
                    <span>•</span>
                    <span>جۆر: {repSelectedRoundObj?.type || '---'}</span>
                    <span>•</span>
                    <span>کۆی دەنگدەرانی خول: {repSelectedRoundObj?.totalVoters.toLocaleString() || '0'}</span>
                  </div>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
                  <div className="space-y-4">
                    <h4 className="font-bold text-base border-s-4 border-emerald-500 ps-3">پوختەی هەڵسەنگاندن و دەنگەکان</h4>
                    <div className="bg-[var(--bg-main)] p-4 rounded-xl border border-[var(--border-color)] space-y-3 text-sm">
                      <div className="flex justify-between items-center border-b border-[var(--border-color)] pb-2">
                        <span className="text-[var(--text-secondary)]">کۆی دەنگی دروستی فلتەرکراو:</span>
                        <span className="font-black text-emerald-600 text-base">{totalReportVotes.toLocaleString()} دەنگ</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-[var(--text-secondary)]">ژمارەی لقە بەشداربووەکان:</span>
                        <span className="font-bold">{repSelectedBranchIds.length === 0 ? repFilteredBranches.length : repSelectedBranchIds.length} لق</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col items-center justify-center">
                    <div className="w-56 h-56 relative">
                      <svg viewBox="0 0 100 100" className="w-full h-full drop-shadow">
                        {createSvgSlices(reportData, true)}
                      </svg>
                    </div>
                    <span className="text-xs font-bold text-[var(--text-secondary)] mt-2">دیاریکردنی شێوازی دابەشبوونی دەنگەکان</span>
                  </div>
                </div>

                <div className="space-y-4">
                  <h4 className="font-bold text-base border-s-4 border-blue-500 ps-3">خشتەی دروستی دەنگی لایەنەکان</h4>
                  <div className="overflow-x-auto">
                    <table className="w-full text-start border-collapse border border-[var(--border-color)] text-sm">
                      <thead>
                        <tr className="bg-[var(--bg-main)] border-b border-[var(--border-color)] text-[var(--text-secondary)]">
                          <th className="py-3 px-4 border-e border-[var(--border-color)]">#</th>
                          <th className="py-3 px-4 border-e border-[var(--border-color)]">{t.partyNameHeader}</th>
                          <th className="py-3 px-4 border-e border-[var(--border-color)]">{t.votesCountLabel}</th>
                          <th className="py-3 px-4">{t.percentageLabel}</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-[var(--border-color)]">
                        {reportData.map((item, idx) => (
                          <tr key={item.partyId} className="hover:bg-[var(--bg-hover)]">
                            <td className="py-2.5 px-4 font-bold border-e border-[var(--border-color)] text-center">{idx + 1}</td>
                            <td className="py-2.5 px-4 font-bold border-e border-[var(--border-color)]">
                              <div className="flex items-center gap-2">
                                <span className="w-3.5 h-3.5 rounded-full" style={{ backgroundColor: item.hexColor }}></span>
                                <span>{item.partyName}</span>
                              </div>
                            </td>
                            <td className="py-2.5 px-4 font-bold border-e border-[var(--border-color)]">{item.votes.toLocaleString()}</td>
                            <td className="py-2.5 px-4 font-extrabold text-blue-600">{item.percentage}%</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                ...
                <div className="pt-8 border-t border-[var(--border-color)] flex justify-between items-end text-xs text-[var(--text-secondary)]">
                  <div>
                    <p>چاپکراوە لە لایەن: <span className="font-bold text-[var(--text-primary)]">{currentUser.name}</span></p>
                    <p>بەرواری دەرچوون: {new Date().toLocaleDateString('en-GB')}</p>
                  </div>
                  <div className="text-center space-y-8">
                    <p className="font-bold">مۆر و ئیمزای بەڕێوەبەرایەتی</p>
