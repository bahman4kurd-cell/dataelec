      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="flex-1 p-6 overflow-y-auto space-y-6">
              ...
              {currentUser.role === 'super_admin' && (
                <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-6 shadow-sm space-y-4">
                  ...
                  <form onSubmit={handleCreateAccount} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                    <div>
                      <label className="block text-xs font-medium mb-1 text-[var(--text-secondary)]">{t.fullNameLabel}</label>
                      <input
                        type="text"
                        value={newAccName}
                        onChange={(e) => setNewAccName(e.target.value)}
                        placeholder="بەهمەن محەمەد"
                        className="w-full bg-[var(--bg-main)] border border-[var(--border-color)] rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500"
                        required
                      />
                    </div>
                    ...
                    <div>
                      <label className="block text-xs font-medium mb-1 text-[var(--text-secondary)]">{t.username}</label>
                      <input
                        type="text"
                    ...
                    <div>
                      <label className="block text-xs font-medium mb-1 text-[var(--text-secondary)]">{t.password}</label>
                      <input
                        type="text"
                    ...
                    <div>
                      <label className="block text-xs font-medium mb-1 text-[var(--text-secondary)]">{t.roleLabel}</label>
                      <select
                        value={newAccRole}
                    ...
                    <div>
                      <label className="block text-xs font-medium mb-1 text-[var(--text-secondary)]">{t.linkedBranchHeader}</label>
                      <select
                            ...
                            <tr key={u.id} className="hover:bg-[var(--bg-hover)]">
                              ...
                              <td className="py-3 px-4">
                                <input
                                  ...
                                  className="bg-[var(--bg-main)] border border-[var(--border-color)] rounded px-2 py-1 text-xs w-28 focus:ring-2 focus:ring-blue-500 font-mono"
                                />
                              </td>
                              <td className="py-3 px-4">
                                <span className={`px-2 py-0.5 rounded text-xs font-bold ${
                                  u.role === 'super_admin' ? 'bg-purple-100 text-purple-800' : u.role === 'branch_admin' ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800'
                                }`}>
                                  {u.role}
                                </span>
                              </td>
          ...
          {activeMainTab === 'reports' && currentUser.role !== 'viewer' && (
            <div className="space-y-6">
              <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-6 shadow-sm space-y-4 print:hidden">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-[var(--border-color)] pb-4">
                  <div>
                    ...
                    <p className="text-sm text-[var(--text-secondary)]">{t.reportsSubtitle}</p>
                  </div>
                  <div className="flex items-center gap-3">
                    <button
                      onClick={handlePrintReport}
                      className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm font-semibold transition shadow"
                    >
                    ...
                    <button
                      ...
                      className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-lg text-sm font-semibold transition shadow"
                    >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2">
                  <div>
                    <label className="block text-sm font-medium mb-1 text-[var(--text-secondary)]">دیاریکردنی خول</label>
                    <select
                    ...
                    {isRepRegionOpen && (
                      <div className="absolute z-20 mt-1 w-full bg-[var(--bg-card)] border border-[var(--border-color)] rounded-lg shadow-xl p-3 space-y-2 max-h-60 overflow-y-auto">
                        ...
                        {repFilteredRegions.length === 0 ? (
                          <p className="text-xs text-gray-500">هیچ ناوچەیەک نییە</p>
                        ) : (
              ...
              {/* ناوەرۆکی ڕاپۆرتەکە کە بۆ پرنت و PDF دەپێچرێتەوە */}
              <div ref={reportRef} className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-8 shadow-sm space-y-6 print:border-none print:shadow-none print:p-0">
                <div className="text-center border-b border-[var(--border-color)] pb-6 space-y-2">
                  <h1 className="text-2xl font-extrabold">{t.systemTitle}</h1>
                  <h2 className="text-lg font-bold text-blue-600">
                    ڕاپۆرتی کۆتایی ئەنجامەکان: {repSelectedRoundObj?.name || 'دیاری نەکراوە'}
                  </h2>
                  <p className="text-xs text-[var(--text-secondary)]">
                    بەرواری دەرهێنانی ڕاپۆرت: {new Date().toLocaleDateString('fa-IR')}
                  </p>
                </div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-start border-collapse border border-[var(--border-color)]">
                    <thead>
                      <tr className="bg-[var(--bg-main)] border-b border-[var(--border-color)] text-[var(--text-primary)] text-sm">
                        <th className="py-3 px-4 text-start border-e border-[var(--border-color)]">#</th>
                        <th className="py-3 px-4 text-start border-e border-[var(--border-color)]">{t.partyNameHeader}</th>
                        <th className="py-3 px-4 text-center border-e border-[var(--border-color)]">{t.votesCountLabel}</th>
                        <th className="py-3 px-4 text-center">{t.percentageLabel}</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-[var(--border-color)] text-sm">
                      {reportData.map((item, idx) => (
                        <tr key={item.partyId} className="hover:bg-[var(--bg-hover)]">
                          <td className="py-3 px-4 border-e border-[var(--border-color)] font-bold">{idx + 1}</td>
                          <td className="py-3 px-4 border-e border-[var(--border-color)] font-semibold flex items-center gap-2">
                            <span className="w-3.5 h-3.5 rounded-full border border-gray-300" style={{ backgroundColor: item.hexColor }}></span>
                            <span>{item.partyName}</span>
                          </td>
                          <td className="py-3 px-4 border-e border-[var(--border-color)] text-center font-bold text-blue-600">
                    ...
                    <tfoot>
                      <tr className="bg-[var(--bg-main)] font-extrabold text-sm border-t border-[var(--border-color)]">
                        <td colSpan={2} className="py-3 px-4 border-e border-[var(--border-color)] text-end">{t.totalVotesLabel}</td>
                        <td className="py-3 px-4 border-e border-[var(--border-color)] text-center text-blue-600">
                          {totalReportVotes.toLocaleString()}
                        </td>
                        <td className="py-3 px-4 text-center text-emerald-600">100%</td>
                      </tr>
                    </tfoot>
                  </table>
                </div>

                <div className="pt-6 border-t border-[var(--border-color)] flex justify-between items-end text-xs text-[var(--text-secondary)]">
                  <div>
                    <p>مۆری فەرمی / ئیمزای بەرپرس</p>
                    <div className="h-12"></div>
                  </div>
                  <div>
                    <p>سیستەمی شیکاری ئەنجامەکانی هەڵبژاردن v4.0</p>
                  </div>
                </div>
              </div>
            </div>
