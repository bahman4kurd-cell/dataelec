                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="md:col-span-2 space-y-4">
                      {selectedBranchId ? (
                        ...
                        <p className="text-sm text-[var(--text-secondary)] text-center py-8">تکایە لقێک هەڵبژێرە.</p>
                      )}
                    </div>
                  </div>
                </div>
            ...
            <div className="space-y-6">
              {currentUser.role === 'super_admin' && (
                <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-6 shadow-sm space-y-6">
                  ...
                  <form onSubmit={handleCreateAccount} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 bg-[var(--bg-main)] p-4 rounded-xl border border-[var(--border-color)]">
                    <div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full text-start border-collapse">
                        <thead>
                          <tr className="bg-[var(--bg-main)] border-b border-[var(--border-color)] text-xs text-[var(--text-secondary)]">
                            <th className="p-3 text-start">{t.fullNameLabel}</th>
                          ...
                          {users.map(u => {
                            ...
                            return (
                              <tr key={u.id} className="hover:bg-[var(--bg-hover)]">
                                ...
                                <td className="p-3">{bObj ? bObj.name : t.generalOrNone}</td>
                                <td className="p-3">
                                  <input
                                    type="text"
                  ...
                  <div className="border-t border-[var(--border-color)] pt-6 space-y-4">
                    <h3 className="text-lg font-bold">بەڕێوەبردنی لایەنە بەشداربووەکان</h3>
                    
                    <form onSubmit={handleAddNewParty} className="flex gap-4 items-end bg-[var(--bg-main)] p-4 rounded-xl border border-[var(--border-color)]">
                      <div className="flex-1">
                        <label className="block text-xs font-semibold text-[var(--text-secondary)] mb-1">ناوی لایەن / قەوارەی نوێ</label>
                        <input
                          type="text"
                          value={newPartyName}
                      ...
                      <div>
                        <label className="block text-xs font-semibold text-[var(--text-secondary)] mb-1">ڕەنگ</label>
                        <input
                          type="color"
                          value={newPartyColor}
                          onChange={(e) => setNewPartyColor(e.target.value)}
                          className="w-16 h-10 bg-[var(--bg-card)] border border-[var(--border-color)] rounded-lg cursor-pointer p-1"
                        />
                      </div>
                      <button
                        type="submit"
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 max-h-60 overflow-y-auto">
                      {defaultParties.map(party => (
                        <div key={party.partyId} className="flex items-center justify-between gap-2 bg-[var(--bg-main)] p-2.5 rounded-xl border border-[var(--border-color)]">
                          <div className="flex items-center gap-2 truncate">
                            <span className="w-3.5 h-3.5 rounded-full shrink-0" style={{ backgroundColor: party.hexColor }}></span>
                            <span className="text-xs font-semibold truncate">{party.partyName}</span>
                          </div>
                          <button
                  ...
                  <form onSubmit={handleUpdateMyPassword} className="space-y-4">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full bg-[var(--bg-main)] border border-[var(--border-color)] rounded-xl px-4 py-2.5 text-sm focus:outline-none"
                        placeholder="••••••"
                        required
                      />
                    </div>
          ...
          {activeMainTab === 'reports' && currentUser.role !== 'viewer' && (
            <div className="space-y-6">
              <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-6 shadow-sm space-y-6 print:hidden">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-[var(--bg-main)] p-4 rounded-xl border border-[var(--border-color)]">
                  <div>
                    <label className="block text-xs font-semibold text-[var(--text-secondary)] mb-1">خولی هەڵبژاردن</label>
                    <select
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      {isRepBranchOpen && (
                        <div className="absolute z-20 mt-1 w-full bg-[var(--bg-card)] border border-[var(--border-color)] rounded-lg shadow-lg p-2 space-y-1">
                          <div className="flex items-center justify-between pb-1 border-b border-[var(--border-color)] text-xs">
                            <button onClick={() => setRepSelectedBranchIds(repFilteredBranches.map(b => b.id))} className="text-blue-500 hover:underline">هەمووی</button>
                            <button onClick={() => setRepSelectedBranchIds([])} className="text-red-400 hover:underline">پاککردنەوە</button>
                          </div>
                          <div className="max-h-40 overflow-y-auto space-y-1">
                            {repFilteredBranches.map(b => (
                              <label key={b.id} className="flex items-center gap-2 text-xs cursor-pointer p-1 hover:bg-[var(--bg-hover)] rounded">
                                <input
                                  ...
                                  onChange={(e) => {
                                    if (e.target.checked) setRepSelectedBranchIds([...repSelectedBranchIds, b.id]);
                                    else setRepSelectedBranchIds(repSelectedBranchIds.filter(id => id !== b.id));
                                  }}
                                  className="rounded bg-[var(--bg-main)] text-blue-600"
                                />
                                <span>{b.name}</span>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      {isRepRegionOpen && (
                        <div className="absolute z-20 mt-1 w-full bg-[var(--bg-card)] border border-[var(--border-color)] rounded-lg shadow-lg p-2 space-y-1">
                          <div className="flex items-center justify-between pb-1 border-b border-[var(--border-color)] text-xs">
                            <button onClick={() => setRepSelectedRegionIds(repFilteredRegions.map(r => r.id))} className="text-blue-500 hover:underline">هەمووی</button>
                            <button onClick={() => setRepSelectedRegionIds([])} className="text-red-400 hover:underline">پاککردنەوە</button>
                          </div>
                          <div className="max-h-40 overflow-y-auto space-y-1">
                            {repFilteredRegions.map(reg => (
                              <label key={reg.id} className="flex items-center gap-2 text-xs cursor-pointer p-1 hover:bg-[var(--bg-hover)] rounded">
                                <input
                                  ...
                                  onChange={(e) => {
                                    if (e.target.checked) setRepSelectedRegionIds([...repSelectedRegionIds, reg.id]);
                                    else setRepSelectedRegionIds(repSelectedRegionIds.filter(id => id !== reg.id));
                                  }}
                                  className="rounded bg-[var(--bg-main)] text-blue-600"
                                />
                                <span>{reg.name}</span>
              ...
              <div ref={reportRef} className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-8 shadow-sm space-y-6 print:border-none print:shadow-none print:p-2">
                <div className="text-center space-y-2 border-b border-[var(--border-color)] pb-6">
                  <h2 className="text-2xl font-bold">ڕاپۆرتی فەرمی ئەنجامی هەڵبژاردن</h2>
                  <p className="text-sm text-[var(--text-secondary)]">
                    {repSelectedRoundObj ? `${repSelectedRoundObj.name} - بەروار: ${repSelectedRoundObj.date}` : ''}
                  </p>
                </div>

                <div className="grid grid-cols-2 gap-4 bg-[var(--bg-main)] p-4 rounded-xl border border-[var(--border-color)] text-sm">
                  <div>
                    <span className="text-[var(--text-secondary)]">کۆی گشتی دەنگەکان:</span>
                    <span className="font-bold ms-2">{totalReportVotes.toLocaleString()} دەنگ</span>
                  </div>
                  <div>
                    <span className="text-[var(--text-secondary)]">جۆری خول:</span>
                    <span className="font-bold ms-2">{repSelectedRoundObj?.type}</span>
                  </div>
                </div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-start border-collapse border border-[var(--border-color)]">
                    <thead>
                      <tr className="bg-[var(--bg-main)] border-b border-[var(--border-color)] text-xs text-[var(--text-secondary)]">
                        <th className="p-3 border border-[var(--border-color)] text-start">#</th>
                        <th className="p-3 border border-[var(--border-color)] text-start">{t.partyNameHeader}</th>
                        <th className="p-3 border border-[var(--border-color)] text-start">{t.votesCountLabel}</th>
                        <th className="p-3 border border-[var(--border-color)] text-start">{t.percentageLabel}</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-[var(--border-color)] text-sm">
                      {reportData.map((item, idx) => (
                        <tr key={item.partyId} className="hover:bg-[var(--bg-hover)]">
                          <td className="p-3 border border-[var(--border-color)]">{idx + 1}</td>
                          <td className="p-3 border border-[var(--border-color)] font-semibold flex items-center gap-2">
                            <span className="w-3 h-3 rounded-full shrink-0" style={{ backgroundColor: item.hexColor }}></span>
                            <span>{item.partyName}</span>
                          </td>
                          <td className="p-3 border border-[var(--border-color)]">{item.votes.toLocaleString()}</td>
                          <td className="p-3 border border-[var(--border-color)] font-bold">{item.percentage}%</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                <div className="pt-12 flex justify-between text-xs text-[var(--text-secondary)] border-t border-[var(--border-color)]">
                  <div>واژۆی بەرپرسی لێژنەی باڵا</div>
