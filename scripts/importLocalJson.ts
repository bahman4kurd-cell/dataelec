...
function parseArgs(argv: string[]): Args {
  ...
  if (!jsonPath) {
    throw new Error('شێوازی بەکارهێنان: npm run migrate:json -- <path-to-json> [--slug=main] [--replace=true|false]');
  }
...
async function run(): Promise<void> {
  ...
  if (!existsSync(args.jsonPath)) {
    throw new Error(`فایلی JSON لەم شوێنە دۆزنەکرایەوە: ${args.jsonPath}`);
  }
  ...
  if (!parsed.success) {
    throw new Error(`پێکهاتەی فایلی JSON هەڵەیە: ${JSON.stringify(parsed.error.flatten())}`);
  }
  ...
  try {
    ...
    if (args.replace) {
      await client.query(`DELETE FROM election_rounds WHERE instance_id = $1`, [instanceId]);
      await client.query(`DELETE FROM parties WHERE instance_id = $1`, [instanceId]);
    }
        ...
        for (let subtabIndex = 0; subtabIndex < branch.subTabs.length; subtabIndex += 1) {
          ...
          const subtabResult = await client.query<{ id: string }>(
            ...
            INSERT INTO election_subtabs (
              branch_id, external_id, name, total_cast_votes, burned_votes,
              valid_votes, auto_calc_valid_votes, selected_chart_type, notes, display_order
            )
    ...
    console.log('✓ پرۆسەی هێنانی داتای JSON بەسەرکەوتوویی تەواو بوو. - importLocalJson.ts:363');
    console.log(`Instance Slug: ${args.slug} - importLocalJson.ts:364`);
    console.log(`خولەکان: ${data.rounds.length} - importLocalJson.ts:365`);
    console.log(`لایەنەکان: ${partySeedMap.size} - importLocalJson.ts:366`);
  } catch (error) {
    await client.query('ROLLBACK');
    console.error('✕ هێنانی داتای JSON شکستی هێنا: - importLocalJson.ts:369', error);
    process.exitCode = 1;
  } finally {
    client.release();
    await pool.end();
